...
//
#define IDM_MING_CARD_STATE				(WM_USER+107)						//明牌开始
#define IDM_MING_CARD				(WM_USER+108)						//明牌
#define IDM_QIANG_LAND				(WM_USER+109)						//抢地主
#define IDM_MING_OUT				(WM_USER+110)						//出牌明牌
#define IDM_ADD						(WM_USER+111)						//加倍
...
protected:
	...
	BYTE							m_bLaiZiCard;						//赖子扑克
	bool							m_bLaiZiDisplay;					//显示赖子
	BYTE							m_bRandCardX;
	BYTE							m_bRandCardY;
...
public:
	...
	bool							m_bAdd[GAME_PLAYER];				//加倍信息
	bool							m_bAddStates[GAME_PLAYER];			//加倍状态
	bool							m_bLaiZi;							//赖子框
...
public:
	...
	//位图变量
protected:
	CBitImage						m_ImageBomb;						//炸弹图片
	CBitImage						m_ImageBack;						//背景资源
	CBitImage						m_ImageCenter;						//背景资源
	CBitImage						m_ImageBombEffect;					//炸弹效果
	CBitImage						m_ImageHistoryScore;				//历史积分
	CBitImage						m_ImageLaiZiCard;					//赖子动画

	CPngImage						m_ImageScore;						//分数图片
	CPngImage						m_ImageLand;						//庄家图片
	CPngImage						m_ImageUserTrustee;					//玩家托管
	CPngImage						m_ImageDouble;						//加倍
	CPngImage						m_ImageNumber;						//数字
	CPngImage						m_ImagePlayerRL;						//用户框
	CPngImage						m_ImagePlayerTB;						//用户框
	CPngImage						m_ImageLaiZi;						//赖子框
...
public:
	...
	//
	CSkinButton						m_btMingCardState;						//明牌开始
	CSkinButton						m_btMingCard;						//明牌
	CSkinButton						m_btQiangLand;						//抢地主
	CSkinButton						m_btNoQiang;						//不抢
	CSkinButton						m_btMingOut;						//出牌明牌
	CSkinButton						m_btAdd;							//加倍
	CSkinButton						m_btNoAdd;							//不加
