	...
	case GER_NORMAL:		//常规结束
		{
			...
			{
				//计算分差
				LONGLONG lUserScore[2];
				IServerUserItem * pIServerUserItem1=m_pITableFrame->GetTableUserItem(0);
				IServerUserItem * pIServerUserItem2=m_pITableFrame->GetTableUserItem(1);
				lUserScore[0]=pIServerUserItem1->GetUserInfo()->lScore;
				lUserScore[1]=pIServerUserItem2->GetUserInfo()->lScore;
				LONGLONG lScoreMargin=(lUserScore[0]>lUserScore[1])?(lUserScore[0]-lUserScore[1]):(lUserScore[1]-lUserScore[0]);
				...
				if (wChairID!=INVALID_CHAIR)
				{
					...
					ZeroMemory(ScoreInfoArray,CountArray(ScoreInfoArray));
					for (WORD i=0;i<m_wPlayerCount;i++)
					{
						ScoreInfoArray[i].cbType =(wWinUser == 0 ? SCORE_TYPE_WIN : SCORE_TYPE_LOSE);
						ScoreInfoArray[i].lRevenue=LONGLONG(LONGLONG(GameEnd.lUserScore[i])*LONGLONG(m_pGameServiceOption->wRevenueRatio)/1000L);;                                  
						ScoreInfoArray[i].lScore=GameEnd.lUserScore[i];
					}
	...
	case GER_NETWORK_ERROR:	//网络中断
		{
			...
			if (m_GameRuleInfo.cbGameMode==GM_MATCH)
			{
				...
				ScoreInfoArray[wChairID].lScore=GameEnd.lUserScore[wChairID];
	...
	case GAME_STATUS_FREE:		//空闲状态
		{
			...
			if (pUserData->cbUserStatus!=US_LOOKON)
			{
				...
				{
					if (m_pITableFrame->GetTableUserItem(i)!=NULL) wUserCount++;
				}
				if (wUserCount==1)
				{
					m_bGameRule=false;
					m_wBlackUser=wChairID;
				}
			}
	...
	case GAME_STATUS_PLAY:	//游戏状态
		{
			...
			StatusPlay.wTimeOutCount=m_wTimeOutCount[wChairID];
	...
	case SUB_C_PLACE_CHESS:		//放置棋子
		{
			...
			//消息处理
			CMD_C_PlaceChess * pPlaceChess=(CMD_C_PlaceChess *)pData;
	...
	case SUB_C_REGRET_ANSWER:	//悔棋应答
		{
			...
			//消息处理
			CMD_C_RegretAnswer * pRegretAnswer=(CMD_C_RegretAnswer *)pData;
	...
	case SUB_C_PEACE_ANSWER:	//求和应答
		{
			...
			//消息处理
			CMD_C_PeaceAnswer * pPeaceAnswer=(CMD_C_PeaceAnswer *)pData;
...
bool  CTableFrameSink::OnFrameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	case SUB_GF_USER_READY:		//用户同意
		{
			...
			if (pUserData->wChairID==m_wBlackUser)
			{
				...
				if (m_pITableFrame->GetTableUserItem(wAgreeChairID)!=NULL)
				{
	...
	case SUB_GF_USER_CHAT:	//用户聊天
		{
			//游戏状态
			if (m_pITableFrame->GetGameStatus()!=GAME_STATUS_PLAY) return false;
			...
			if (m_GameRuleInfo.cbDirections==FALSE)
			{
				//权限判断
				tagUserInfo * pUserData=pIServerUserItem->GetUserInfo();
				if (pUserData->cbMasterOrder!=0) return false;

				//发送消息
				LPCTSTR pszMessage=TEXT("根据玩家规则设定，游戏正在进行中，禁止交谈活动");
				m_pITableFrame->SendGameMessage(pIServerUserItem,pszMessage,SMT_CHAT);
...
bool  CTableFrameSink::OnActionUserStandUp(WORD wChairID,IServerUserItem * pIServerUserItem, bool bLookonUser)
{
	...
	if ((bLookonUser==false)&&(wChairID==m_wBlackUser))
	{
		...
		//请求设置
		if (m_pITableFrame->GetTableUserItem(m_wBlackUser)!=NULL)
		{
			CMD_S_Req_SetRult SetRult;
			SetRult.GameRuleInfo=m_GameRuleInfo;
			m_pITableFrame->SendTableData(m_wBlackUser,SUB_S_REQ_SET_RULE,&SetRult,sizeof(SetRult));
...
bool CTableFrameSink::OnUserTimeOut(WORD wChairID)
{
	...
	else OnEventGameConclude((wChairID+1)%GAME_PLAYER,NULL,GER_NORMAL);
...
bool CTableFrameSink::OnUserPeaceAnswer(WORD wChairID, BYTE cbApprove)
{
	...
	else OnEventGameConclude(INVALID_CHAIR,NULL,GER_NORMAL);
...
bool CTableFrameSink::OnUserChessCount(WORD wChairID)
{
	...
	if ((m_cbBegStatus[0]&GU_WAIT_CHESS_COUNT)&&(m_cbBegStatus[1]&GU_WAIT_CHESS_COUNT))
	{
		...
		if (wWinUser!=INVALID_CHAIR) pIServerUserItem=m_pITableFrame->GetTableUserItem(wWinUser);

		//结束游戏
		OnEventGameConclude(wWinUser,pIServerUserItem,GER_NORMAL);
	}
	else
	{
		//发送消息
