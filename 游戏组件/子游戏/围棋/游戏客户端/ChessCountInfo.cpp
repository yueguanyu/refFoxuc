...
CChessCountInfo::CChessCountInfo() : CDialog(IDD_CHESS_COUNT)
{
	...
	//加载资源
	m_ImageBack.LoadFromResource(AfxGetInstanceHandle(),IDB_CHESS_COUNT_INFO);
...
BOOL CChessCountInfo::OnInitDialog()
{
	...
	//设置按钮
	m_btOk.SetButtonImage(IDB_COUNT_BT_ACCEPT,AfxGetInstanceHandle(),false,false);
	m_btCancel.SetButtonImage(IDB_COUNT_BT_CONTINUE,AfxGetInstanceHandle(),false,false);
...
void CChessCountInfo::OnOK()
{
	...
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_CHESS_COUNT_RESPONSION,1,1);
...
void CChessCountInfo::OnCancel()
{
	//发送消息
	CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_CHESS_COUNT_RESPONSION,0,0);
...
void CChessCountInfo::OnPaint()
{
	...
	//绘画信息
	TCHAR szBuffer[64];
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%0.1f"),m_fBlackCount);
	dc.TextOut(133,57,szBuffer,lstrlen(szBuffer));
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%0.1f"),m_fWhiteCount);
	dc.TextOut(133,83,szBuffer,lstrlen(szBuffer));
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%0.1f"),m_fCommomCount);
