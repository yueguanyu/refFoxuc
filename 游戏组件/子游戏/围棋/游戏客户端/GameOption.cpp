...
CGameOption::CGameOption() : CDialog(IDD_OPTION)
{
	...
	//加载资源
	m_ImageBack.LoadFromResource(AfxGetInstanceHandle(),IDB_GAME_OPTION);
...
BOOL CGameOption::OnInitDialog()
{
	...
	//设置按钮
	m_btOK.SetButtonImage(IDB_OPTION_BT_OK,AfxGetInstanceHandle(),false,false);
	m_btCancel.SetButtonImage(IDB_OPTION_BT_CANCEL,AfxGetInstanceHandle(),false,false);
...
void CGameOption::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(WM_NCLBUTTONDOWN,HTCAPTION,MAKELPARAM(point.x,point.y));
