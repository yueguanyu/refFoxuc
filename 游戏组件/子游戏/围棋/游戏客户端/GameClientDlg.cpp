...
bool CGameClientEngine::OnEventGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case SUB_S_PASS_CHESS:		//放弃下棋
		{
			return OnSubPassChess(pData,wDataSize);
		}
	...
	case SUB_S_REQ_SET_RULE:	//请求设置
		{
			return OnSubSetRule(pData,wDataSize);
		}
	case SUB_S_REQ_AGREE_RULE:	//请求同意
		{
			return OnSubAgreeRule(pData,wDataSize);
		}
	case SUB_S_REQ_CHESS_COUNT:	//请求点目
		{
			return OnSubReqChessCount(pData,wDataSize);
		}
	case SUB_S_UNAGREE_CHESS_COUNT:	//点目应答
		{
			return OnSubChessCountResponsion(pData,wDataSize);
		}
	}
	...
	case GAME_STATUS_FREE:		//空闲状态
		{
			...
			else 
			{
				if(m_pIStringMessage != NULL)
					m_pIStringMessage->InsertSystemString(TEXT("正等待“黑方”玩家设置游戏规则"));
			}
...
//用户放棋
bool CGameClientEngine::OnSubPlaceChess(const void * pBuffer, WORD wDataSize)
{
	//效验数据
...
//放弃下棋
bool CGameClientEngine::OnSubPassChess(const void * pBuffer, WORD wDataSize)
{
	//效验数据
...
//悔棋请求
bool CGameClientEngine::OnSubRegretReq(const void * pBuffer, WORD wDataSize)
{
...
//悔棋失败
bool CGameClientEngine::OnSubRegretFaile(const void * pBuffer, WORD wDataSize)
{
	//效验数据
	...
	if (pRegretFaile->cbFaileReason==FR_PLAYER_OPPOSE)
	{
		WORD wMeChairID=GetMeChairID();
		if(m_pIStringMessage != NULL)
			m_pIStringMessage->InsertSystemString(TEXT("对家不同意你 [ 悔棋 ]"));
...
//悔棋结果
bool CGameClientEngine::OnSubRegretResult(const void * pBuffer, WORD wDataSize)
{
	//效验数据
...
//和棋请求
bool CGameClientEngine::OnSubPeaceReq(const void * pBuffer, WORD wDataSize)
{
...
//和棋应答
bool CGameClientEngine::OnSubPeaceAnser(const void * pBuffer, WORD wDataSize)
{
	//设置界面
	m_GameClientView.m_btPeace.EnableWindow(TRUE);
	if(m_pIStringMessage != NULL)
		m_pIStringMessage->InsertSystemString(TEXT("对家不同意你 [ 和棋 ]"));
...
bool CGameClientEngine::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		IClientUserItem * pClientUserItem=GetTableUserItem(wUserChairID);
		if(pClientUserItem == NULL)continue;
		GameScoreInfo.lScore[i]=pGameEnd->lUserScore[wUserChairID];
		lstrcpyn(GameScoreInfo.szUserName[i],pClientUserItem->GetNickName(),CountArray(GameScoreInfo.szUserName[i]));

		//在聊天框显示成绩
		_sntprintf(szBuffer,CountArray(szBuffer),TEXT(" \n%s: %+I64d分"),GameScoreInfo.szUserName[i],pGameEnd->lUserScore[i]);
		if(m_pIStringMessage != NULL)
	...
	if (m_bAutoSaveManual==true)
	{
		...
		_sntprintf(szFileName,sizeof(szFileName),TEXT("%s\\围棋棋谱_%04ld_%02ld_%02ld_%02ld_%02ld_%02ld.WQM"),
			szPath,SystemTime.wYear,SystemTime.wMonth,SystemTime.wDay,SystemTime.wHour,SystemTime.wMinute,SystemTime.wSecond);

		//保存棋谱
		if (SaveChessManual(szFileName)==false)	
		{
			if(m_pIStringMessage != NULL)
				m_pIStringMessage->InsertSystemString(TEXT("围棋游戏“棋谱记录”保存失败"));
		}
		else 
		{
			if(m_pIStringMessage != NULL)
				m_pIStringMessage->InsertSystemString(TEXT("围棋游戏“棋谱记录”保存成功"));
		}
	}
	else
	{
		...
		if (bNotify==false)
		{
			bNotify=true;
			if(m_pIStringMessage != NULL)
				m_pIStringMessage->InsertSystemString(TEXT("游戏结束了，若需要保存棋谱，请点击 [ 保存 ] 按钮保存棋谱记录"));
		}
	}
...
//游戏棋谱
bool CGameClientEngine::OnSubChessManual(const void * pBuffer, WORD wDataSize)
{
	//效验参数
...
//请求设置
bool CGameClientEngine::OnSubSetRule(const void * pBuffer, WORD wDataSize)
{
	//效验数据
...
//请求同意
bool CGameClientEngine::OnSubAgreeRule(const void * pBuffer, WORD wDataSize)
{
	//效验数据
...
//请求点目
bool CGameClientEngine::OnSubReqChessCount(const void * pBuffer, WORD wDataSize)
{
	//设置变量
	...
	ActiveGameFrame();
	if(m_pIStringMessage != NULL)
		m_pIStringMessage->InsertSystemString(TEXT("对方玩家请求“点目”"));

	return true;
}

//点目应答
bool CGameClientEngine::OnSubChessCountResponsion(const void * pBuffer, WORD wDataSize)
{
	//设置变量
	m_bReqChessCount=false;
	...
	//玩家设置
	if(m_pIStringMessage != NULL)
		m_pIStringMessage->InsertSystemString(TEXT("对方玩家不同意你的“点目”请求"));

	return true;
}

//保存棋谱
bool CGameClientEngine::SaveChessManual(LPCTSTR pszFileName)
{
	//打开文件
	CFile FileManual;
		...
		if ((m_bLimitTime==true)&&(m_StatusInfo[m_wCurrentUser].wLimitTimeCount!=0xFFFF))
		{
			...
			if (m_StatusInfo[m_wCurrentUser].wLimitTimeCount==0xFFFF)
			{
				if (m_GameRuleInfo.wRuleTimeOutCount>0)
				{
					...
					{
						//发送消息
						SendSocketData(SUB_C_TIME_OUT);

						//提示消息
						TCHAR szMessage[128];
						_sntprintf(szMessage,sizeof(szMessage),TEXT("下棋超时了，还有 %ld 次 %ld 秒的超时机会"),
				...
				else 
				{
					...
					if ((GetMeChairID()==m_wCurrentUser)&&(IsLookonMode()==false))
					{
						...
						//提示消息
						if(m_pIStringMessage != NULL)
							m_pIStringMessage->InsertSystemString(TEXT("下棋超时，你被判输"));
...
//点目按钮
LRESULT CGameClientEngine::OnCount(WPARAM wParam, LPARAM lParam)
{
	//点目判断
...
//通过按钮
LRESULT CGameClientEngine::OnPass(WPARAM wParam, LPARAM lParam)
{
	//发送消息
	SendSocketData(SUB_C_PASS_CHESS);
...
//悔棋按钮
LRESULT CGameClientEngine::OnRegret(WPARAM wParam, LPARAM lParam)
{
	//发送消息
	SendSocketData(SUB_C_REGRET_REQ);

	//设置界面
	m_GameClientView.m_btRegret.EnableWindow(FALSE);
	if(m_pIStringMessage != NULL)
		m_pIStringMessage->InsertSystemString(TEXT("正在等待对家回应 [ 悔棋 ] 请求..."));

	return 0;
}

//求和按钮
LRESULT CGameClientEngine::OnPeace(WPARAM wParam, LPARAM lParam)
{
	//和棋确认
	...
	//发送消息
	SendSocketData(SUB_C_PEACE_REQ);

	//设置界面
	m_GameClientView.m_btPeace.EnableWindow(FALSE);
	if(m_pIStringMessage != NULL)
		m_pIStringMessage->InsertSystemString(TEXT("正在等待对家回应 [ 和棋 ] 请求..."));
...
LRESULT CGameClientEngine::OnGiveUp(WPARAM wParam, LPARAM lParam)
{
	...
	//发送消息
	SendSocketData(SUB_C_GIVEUP_REQ);
...
//保存按钮
LRESULT CGameClientEngine::OnPreserve(WPARAM wParam, LPARAM lParam)
{
	//获取目录
	TCHAR szPath[MAX_PATH]=TEXT("");
	GetCurrentDirectory(sizeof(szPath),szPath);
...
//研究按钮
LRESULT CGameClientEngine::OnStudy(WPARAM wParam, LPARAM lParam)
{
	//创建棋谱
...
//和棋应答
LRESULT CGameClientEngine::OnPeaceAnswer(WPARAM wParam, LPARAM lParam)
{
	//发送消息
	CMD_C_PeaceAnswer PeaceAnswer;
	PeaceAnswer.cbApprove=BYTE(wParam);
	SendSocketData(SUB_C_PEACE_ANSWER,&PeaceAnswer,sizeof(PeaceAnswer));

	return 0;
}

//悔棋应答
LRESULT CGameClientEngine::OnRegretAnswer(WPARAM wParam, LPARAM lParam)
{
	//设置界面
	if (wParam==TRUE)
	{
	...
	SendSocketData(SUB_C_REGRET_ANSWER,&ReqretAnswer,sizeof(ReqretAnswer));

	return 0;
}

//点击棋盘
LRESULT CGameClientEngine::OnHitChessBorad(WPARAM wParam, LPARAM lParam)
{
	if (m_wCurrentUser==GetMeChairID())
	{
		...
		{
			if(m_pIStringMessage != NULL)
				m_pIStringMessage->InsertSystemString(TEXT("这点是死穴或者禁着点，不能在此点放棋"));
		...
		SendSocketData(SUB_C_PLACE_CHESS,&PlaceChess,sizeof(PlaceChess));
	}
...
//同意规则
LRESULT CGameClientEngine::OnAgreeGameRule(WPARAM wParam, LPARAM lParam)
{
	//规则变量
...
//点目应答
LRESULT CGameClientEngine::OnChessCountResponsion(WPARAM wParam, LPARAM lParam)
{
	//设置界面
	m_GameClientView.m_ChessBorad.CleanCountInfo();
	...
	if ((wParam==TRUE)&&(m_bReqChessCount==false))
	{
		...
		//发送消息
		SendSocketData(SUB_C_REQ_CHESS_COUNT);

		//提示消息
		if(m_pIStringMessage != NULL)
			m_pIStringMessage->InsertSystemString(TEXT("正在对方玩家响应“点目”请求"));
	}
	...
	if ((wParam==FALSE)&&(m_bResponsionChessCount==true))
	{
		...
		//发送消息
		SendSocketData(SUB_C_UNAGREE_CHESS_COUNT);

		//提示消息
		if(m_pIStringMessage != NULL)
			m_pIStringMessage->InsertSystemString(TEXT("你拒绝了对方的“点目”请求"));
	}
