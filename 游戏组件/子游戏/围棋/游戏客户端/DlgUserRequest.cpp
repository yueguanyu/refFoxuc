...
void CDlgUserRequest::OnCancel()
{
	...
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(m_uMessage,0,0);
	DestroyWindow();
...
{
	CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(m_uMessage,TRUE,TRUE);
	DestroyWindow();
...
void CDlgUserRequest::OnTimer(UINT nIDEvent)
{
	if (nIDEvent==IDI_TIME)
	{
		...
		if (m_nResidualTime>0)
		{
			...
			_sntprintf(szTitle,sizeof(szTitle),TEXT("用户请求 -- %ld 秒"),m_nResidualTime);
