...
CGameRule::CGameRule() : CDialog(IDD_GAME_RULE)
{
	...
	//加载资源
	m_ImageBack.LoadFromResource(AfxGetInstanceHandle(),IDB_GAME_RULE);
...
BOOL CGameRule::OnInitDialog()
{
	...
	//设置按钮
	m_btOk.SetButtonImage(IDB_RULE_AGREE,AfxGetInstanceHandle(),false,false);
	m_btCancel.SetButtonImage(IDB_RULE_QUIT,AfxGetInstanceHandle(),false,false);
	...
	{
		_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%d"),wStepTime[i]);
	...
	{
		_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%d"),wTimeCountCount[i]);
		nItem=pComboBox->InsertString(i,szBuffer);
	...
	{
		_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%d"),wLimitTime[i]/60);
		nItem=pComboBox->InsertString(i,szBuffer);
...
void CGameRule::OnOK()
{
	...
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_AGREE_GAME_RULE,0,0);
...
void CGameRule::OnCancel()
{
	//发送消息
	CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(WM_CLOSE,0,0);
