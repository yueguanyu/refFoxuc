...
bool CGameLogic::MoveChess(BYTE cbXSourcePos, BYTE cbYSourcePos, BYTE cbXTargetPos, BYTE cbYTargetPos, tagMoveChessResult & MoveChessResult, LPTSTR pszManualName)
{
	...
	{
		TraceMessage(TEXT("113444444_获取棋谱"));
		return false;
	}
	...
	{
		TraceMessage(TEXT("113444444_获取移动棋子棋谱"));
		return false;
	}
	...
	{
		TraceMessage(TEXT("113444444_插入棋谱"));
		return false;
	}
...
bool CGameLogic::IsGameFinish(BYTE cbMoveColor, tagConcludeInfo & ConcludeInfo)
{
	...
	{
		TraceMessage(TEXT("113444444_(获取将棋,TraceMessage)"));
		return false;
	}
	...
	{
		TraceMessage(TEXT("113444444_(获取将棋,困毙判断)"));
		return false;
	}
	...
	case CHESS_GENERAL:	//将
		{
			...
			{
				TraceMessage(TEXT("113444444_CHESS_GENERAL"));
				return false;
			}
	...
	case CHESS_KAVASS:	//士
		{
			...
			{
				TraceMessage(TEXT("113444444_ CHESS_KAVASS"));
				return false;
			}
	...
	case CHESS_ELEPHANT://象
		{
			...
			{
				TraceMessage(TEXT("113444444_ CHESS_ELEPHANT"));
				return false;
			}
			return true;
		}
	case CHESS_HORSE:	//马
		{
			...
			{
				TraceMessage(TEXT("113444444_ CHESS_HORSE"));
				return false;
			}
			return true;
		}
	case CHESS_ROOK:	//车
		{
			...
			{
				TraceMessage(TEXT("113444444_ CHESS_ROOK"));
				return false;
			}
			return true;
		}
	case CHESS_CANNON:	//炮
		{
			...
			{
				TraceMessage(TEXT("113444444_ CHESS_CANNON"));
				return false;
			}
			...
			{
				TraceMessage(TEXT("113444444_ 阻塞分析"));
				return false;
			}
			return true;
		}
	case CHESS_SOLDIER:	//兵
		{
			...
			{
				TraceMessage(TEXT("113444444_ CHESS_SOLDIER"));
				return false;
			}
...
bool CGameLogic::PerformMoveChess(BYTE cbXSourcePos, BYTE cbYSourcePos, BYTE cbXTargetPos, BYTE cbYTargetPos, tagMoveChessResult & MoveChessResult)
{
	...
	{
		//威胁判断
		tagChessItem * pMinatoryChess=NULL;
		
		try
		{
			pMinatoryChess=GetMinatoryChess(pSourceChessItem->cbXPos,pSourceChessItem->cbYPos);
		}
		catch(...)
		{
			TraceMessage(TEXT("113444444_(效验pMinatoryChess=GetMinatoryChess(pSourTime)"));
			return false;
		}
		...
		{
			//有根判断
			tagChessItem * pProtectedChess=NULL;

			try
			{
				pProtectedChess=GetProtectedChess(pSourceChessItem->cbXPos,pSourceChessItem->cbYPos);
			}
			catch(...)
			{
				TraceMessage(TEXT("113444444_(效验pProtectedChess=pProtectedChess(pSourTime)"));
				return false;
			}
	...
	{
		TraceMessage(TEXT("113444444_(将军判断=将军判断(pSourTime)"));
		return false;
	}
	...
	try
	{
		...
		if ((pTargetChessItem==NULL)&&(pSourceChessItem->cbChess!=CHESS_GENERAL)&&(pSourceChessItem->cbChess!=CHESS_SOLDIER))
		{
			...
			{
				TraceMessage(TEXT("113444444_(将军解救判断军判断(pSourTime)"));
				return false;
			}
			...
			{
				TraceMessage(TEXT("113444444_(捉子判断捉子判断判断军判断()"));
				return false;
			}
			...
			{
				TraceMessage(TEXT("113444444_(3546还原状态66还原状态666()"));
				return false;
			}
		}
		else
		{
			...
			{
				TraceMessage(TEXT("113444444_(还原变量定义状态(变量定义还原状态)"));
				return false;
			}
		}
	}
	...
	{
		TraceMessage(TEXT("113444444_(还原状态(还原状态)"));
		return false;
	}
...
tagChessItem * CGameLogic::GetMinatoryChess(BYTE cbXTargetPos, BYTE cbYTargetPos)
{
	...
	try
	{
		...
		tagChessItem * pTargetChessItem=m_ChessBorad[cbXTargetPos][cbYTargetPos];
		if(pTargetChessItem==NULL)return NULL;
	...
	{
		TraceMessage(TEXT("113444444_(return return NULL;;,困毙判断)"));
		return false;
	}
	return NULL;
}
