...
CChessBorad::CChessBorad()
{
	...
	//加载资源
	m_ImageWaitGameRule.LoadFromResource(AfxGetInstanceHandle(),IDB_WAIT_GAME_RULE);
...
VOID CChessBorad::DrawChessBorad(CDC * pDC)
{
	...
	//m_ImageChessBorad.TransDrawImage(pDC,m_BenchmarkPos.x,m_BenchmarkPos.y, RGB(255,0,255));
	...
	for (BYTE cbXPos=0;cbXPos<COUNT_X;cbXPos++)
	{
		for (BYTE cbYPos=0;cbYPos<COUNT_Y;cbYPos++)
		{
			...
			if (bFrame==true) m_ImageChessFrame.TransDrawImage(pDC,nXBoradPos,nYBoradPos,RGB(255,0,255));
		}
	}
	...
	if (m_bWaitGameRule==true)
	{
		...
		m_ImageWaitGameRule.TransDrawImage(pDC,m_BenchmarkPos.x+nXPos,m_BenchmarkPos.y+nYPos,RGB(255,0,255));
	}
...
VOID CChessBorad::SetChessBoradMode(bool bSmallMode)
{
	...
	if (bSmallMode==false)
	{
		...
		m_ImageChessFrame.LoadFromResource(AfxGetInstanceHandle(),IDB_CHESS_FRAME);
		m_ImageChessBorad.LoadFromResource(AfxGetInstanceHandle(),IDB_CHESS_BORAD);
	...
	else
	{
		...
		m_ImageChessFrame.LoadFromResource(AfxGetInstanceHandle(),IDB_CHESS_FRAME_S);
		m_ImageChessBorad.LoadFromResource(AfxGetInstanceHandle(),IDB_CHESS_BORAD_S);
...
VOID CChessBorad::OnEventLMouseDown(CPoint Point)
{
	...
	if ((cbXPos!=INVALID_POS)&&(cbYPos!=INVALID_POS)) 
	{
		...
		if (CGameFrameView::GetInstance()!=NULL) CGameFrameView::GetInstance()->SendEngineMessage(WM_HIT_CHESS_BORAD,cbXPos,cbYPos);
	}
