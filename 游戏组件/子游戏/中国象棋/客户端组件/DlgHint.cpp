...
CDlgHint::CDlgHint() : CDialog(IDD_USER_HINT)
{
	...
	//加载资源
	m_ImageViewBack.LoadFromResource(AfxGetInstanceHandle(),IDB_HINT_VIEW);
...
BOOL CDlgHint::OnInitDialog()
{
	...
	m_btOk.SetButtonImage(IDB_BT_HINT_OK,hResInstance,false,false);
...
VOID CDlgHint::OnPaint()
{
	...
	_sntprintf(szTimeLeave,CountChar(szTimeLeave),TEXT("此对话框在 %ld 秒后自动关闭"),m_nResidualTime);
