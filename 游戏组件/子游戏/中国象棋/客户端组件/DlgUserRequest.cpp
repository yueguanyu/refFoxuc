...
CDlgUserRequest::CDlgUserRequest() : CDialog(IDD_USER_REQUEST)
{
	...
	//加载资源
	m_ImageViewBack.LoadFromResource(AfxGetInstanceHandle(),IDB_REQUEST_VIEW);
...
BOOL CDlgUserRequest::OnInitDialog()
{
	...
	m_btOk.SetButtonImage(IDB_REQUEST_BT_OK,hResInstance,false,false);
	m_btCancel.SetButtonImage(IDB_REQUEST_BT_CANCEL,hResInstance,false,false);
...
VOID CDlgUserRequest::OnOK()
{
	...
	if (CGameFrameView::GetInstance()!=NULL) CGameFrameView::GetInstance()->SendEngineMessage(IDM_USER_REQUEST_RESPOND,m_cbRequestType,TRUE);
...
VOID CDlgUserRequest::OnCancel()
{
	...
	if (CGameFrameView::GetInstance()!=NULL) CGameFrameView::GetInstance()->SendEngineMessage(IDM_USER_REQUEST_RESPOND,m_cbRequestType,FALSE);
...
VOID CDlgUserRequest::OnPaint()
{
	...
	if (m_cbRequestType==REQUEST_PEACE)
	{
		...
		RequestStringImage.TransDrawImage(&DCBuffer,(rcClient.Width()-nImageWidth)/2,88,RGB(255,0,255));
	}
