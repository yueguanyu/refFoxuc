	...
	case GAME_SCENE_FREE:	//空闲状态
		{
			...
			if ((bPlayerMode==true)&&((m_bRequestAgree==true)||(m_bRequestCustomize==true)))
			{
				...
				if (pDlgRuleOption->m_hWnd==NULL)
				{
					pDlgRuleOption->Create(IDD_RULE_OPTION,&m_GameClientView);
					pDlgRuleOption->SetWindowSink(&m_GameClientView);
				}
				//显示窗口
			...
			else
			{
				...
				if (bPlayerMode==false)
				{
					...
					if(m_pIStringMessage!=NULL)
						m_pIStringMessage->InsertSystemString(TEXT("正等待玩家设置游戏规则"));
				}
				else
				{
					m_GameClientView.m_ChessBorad.SetWaitGameRule(true);
					...
					if(m_pIStringMessage!=NULL)
						m_pIStringMessage->InsertSystemString(TEXT("正等待“红方”玩家设置游戏规则"));
				}
	...
	case GAME_SCENE_PLAY:	//游戏状态
		{
			...
			for (WORD i=0;i<GAME_PLAYER;i++)
			{
				//const tagUserInfo * pUserData=GetTableUserItem(i)->GetUserInfo();
				IClientUserItem * pIClientUserItem=GetTableUserItem(i);
				lstrcpyn(m_szUserAccounts[i],pIClientUserItem->GetNickName(),CountArray(m_szUserAccounts[i]));
			}
...
//更新按钮
bool CGameClientEngine::UpdateManualControl(WORD wViewStepCount)
{
	//设置变量
...
//执行走棋
bool CGameClientEngine::PerformMoveChess(BYTE cbXSourcePos, BYTE cbYSourcePos, BYTE cbXTargetPos, BYTE cbYTargetPos)
{
	//获取棋子
...
//移动棋子
bool CGameClientEngine::OnSubMoveChess(const VOID * pData, WORD wDataSize)
{
	//效验数据
...
//悔棋失败
bool CGameClientEngine::OnSubRegretFaile(const VOID * pData, WORD wDataSize)
{
	//效验数据
	...
	if (pRegretFaile->cbFaileReason==FR_PLAYER_OPPOSE)
	{
		...
		if(m_pIStringMessage!=NULL)
			m_pIStringMessage->InsertSystemString(TEXT("对家拒绝了您的“悔棋”请求"));
		return true;
	}
...
//悔棋结果
bool CGameClientEngine::OnSubRegretResult(const VOID * pData, WORD wDataSize)
{
	//效验数据
...
//悔棋请求
bool CGameClientEngine::OnSubRegretRequest(const VOID * pData, WORD wDataSize)
{
	//创建窗口
...
//和棋响应
bool CGameClientEngine::OnSubPeaceRespond(const VOID * pData, WORD wDataSize)
{
	//设置界面
	m_GameClientView.m_btPeace.EnableWindow(TRUE);
	...
	if(m_pIStringMessage!=NULL)
		m_pIStringMessage->InsertSystemString(TEXT("对家拒绝了您的“和棋”请求"));
	return true;
}

//和棋请求
bool CGameClientEngine::OnSubPeaceRequest(const VOID * pData, WORD wDataSize)
{
	//创建窗口
	CDlgUserRequest * pDlgUserRequest=&m_GameClientView.m_UserRequestPeace;
	if (pDlgUserRequest->m_hWnd==NULL) pDlgUserRequest->Create(IDD_USER_REQUEST,&m_GameClientView);
...
bool CGameClientEngine::OnSubGameEnd(const VOID * pData, WORD wDataSize)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		{
			FLOAT fPlayCount = FLOAT(pIClientUserItem->GetUserWinCount()+pIClientUserItem->GetUserDrawCount()+pIClientUserItem->GetUserLostCount())+1;
			FLOAT fWinCount = FLOAT(pIClientUserItem->GetUserWinCount());
		...
		{
			FLOAT fPlayCount = FLOAT(pIClientUserItem->GetUserWinCount()+pIClientUserItem->GetUserDrawCount()+pIClientUserItem->GetUserLostCount());
			FLOAT fWinCount = FLOAT(pIClientUserItem->GetUserWinCount());
			if((INT)fPlayCount==0) ScoreInfo.fWinRate[i] =0.0f;
			else ScoreInfo.fWinRate[i] = fWinCount/fPlayCount*100;
		}
		...
		lstrcpyn(ScoreInfo.szUserName[i],(LPCTSTR)pIClientUserItem->GetNickName(),CountArray(ScoreInfo.szUserName[i]));
	}
	...
	if(m_pIStringMessage!=NULL)
	{
		...
		for (WORD i=0;i<GAME_PLAYER;i++)
		{
			...
			if (pClientUserItem!=NULL)
			{
				...
				_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%s:%+I64d"),pClientUserItem->GetNickName(),pGameEnd->lGameScore[i]);
				m_pIStringMessage->InsertCustomString(szBuffer,RGB(0,0,0)/*RGB(0,128,255)*/,RGB(255,255,255));
			}
...
//请求设置
bool CGameClientEngine::OnSubRequestRule(const VOID * pData, WORD wDataSize)
{
	//效验数据
...
//请求同意
bool CGameClientEngine::OnSubRequestAgree(const VOID * pData, WORD wDataSize)
{
	//效验数据
	ASSERT(wDataSize==sizeof(CMD_S_RequestRule));
	if (wDataSize!=sizeof(CMD_S_RequestRule)) return false;
...
//游戏棋谱
bool CGameClientEngine::OnSubChessManual(const VOID * pData, WORD wDataSize)
{
	//效验参数
...
//求和消息
LRESULT CGameClientEngine::OnMessagePeace(WPARAM wParam, LPARAM lParam)
{
	//发送消息
	SendSocketData(SUB_C_PEACE_REQUEST);
...
//悔棋消息		 
LRESULT CGameClientEngine::OnMessageRegret(WPARAM wParam, LPARAM lParam)
{
	//发送消息
	SendSocketData(SUB_C_REGRET_REQUEST);
...
LRESULT CGameClientEngine::OnMessageGiveUp(WPARAM wParam, LPARAM lParam)
{
	...
	SendSocketData(SUB_C_CONCLUDE_REQUEST,&ConcludeRequest,sizeof(ConcludeRequest));

	return 0;
}

//棋谱消息
LRESULT CGameClientEngine::OnMessageManual(WPARAM wParam, LPARAM lParam)
{
	//创建窗口
...
//保存消息		 
LRESULT CGameClientEngine::OnMessagePreserve(WPARAM wParam, LPARAM lParam)
{
	//获取时间
	SYSTEMTIME SystemTime;
	GetLocalTime(&SystemTime);
...
LRESULT CGameClientEngine::OnHitChessBorad(WPARAM wParam, LPARAM lParam)
{
	if (m_wCurrentUser==GetMeChairID())
	{
		...
		if (bWalkLegality==true)
		{
			...
			//发送消息
			SendSocketData(SUB_C_MOVE_CHESS,&MoveChess,sizeof(MoveChess));
...
//同意规则
LRESULT CGameClientEngine::OnGameRuleAgree(WPARAM wParam, LPARAM lParam)
{
	//获取规则
...
//拒绝规则
LRESULT CGameClientEngine::OnGameRuleReject(WPARAM wParam, LPARAM lParam)
{
	//离开游戏
...
//棋谱定位
LRESULT CGameClientEngine::OnOrientationManual(WPARAM wParam, LPARAM lParam)
{
	//显示棋谱
...
//同意请求
LRESULT CGameClientEngine::OnUserRequestRespond(WPARAM wParam, LPARAM lParam)
{
	//同意处理
	switch (wParam)
	{
	case REQUEST_PEACE:		//和棋回应
		{
			...
			//发送消息
			SendSocketData(SUB_C_PEACE_RESPOND,&PeaceRespond,sizeof(PeaceRespond));
	...
	case REQUEST_REGRET:	//悔棋回应
		{
			...
			//发送消息
			SendSocketData(SUB_C_REGRET_RESPOND,&RegretRespond,sizeof(RegretRespond));
