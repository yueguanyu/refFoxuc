...
CDlgRuleOption::CDlgRuleOption() : CDialog(IDD_RULE_OPTION)
{
	...
	//加载资源
	m_ImageViewBack.LoadFromResource(AfxGetInstanceHandle(),IDB_RULE_OPTION_VIEW);
...
BOOL CDlgRuleOption::OnInitDialog()
{
	...
	m_btOk.SetButtonImage(IDB_OPTION_BT_OK,hResInstance,false,false);
	m_btCancel.SetButtonImage(IDB_OPTION_BT_CANCEL,hResInstance,false,false);

	//构造提示
	TCHAR szFastDescribe[128]=TEXT(""),szSlowDescribe[128]=TEXT("");
	_sntprintf(szFastDescribe,CountChar(szFastDescribe),TEXT("局时 %ld 分钟  步时 %ld 分钟  读秒 %ld 秒"),
		MODE_FAST_DRAW_TIME/60L,MODE_FAST_STEP_TIME/60L,MODE_FAST_SECOND_TIME);
	_sntprintf(szSlowDescribe,CountChar(szSlowDescribe),TEXT("局时 %ld 分钟  步时 %ld 分钟  无读秒"),
	...
	LONGLONG lDrawTime[]={600L,1800L,3600L},lStepTime[]={60L,180L,300L};
	LONGLONG lRuleSecondTime[]={30L,60L,180L,300L},lRuleSecondCount[]={1L,3L,5L};

	//游戏局时
	CComboBox * pComboBox=(CComboBox *)GetDlgItem(IDC_DRAW_TIME);
	for (INT i=0;i<CountArray(lDrawTime);i++)
	{
		_sntprintf(szBuffer,CountChar(szBuffer),TEXT("%I64d"),lDrawTime[i]/60L);
		pComboBox->SetItemData(pComboBox->InsertString(i,szBuffer),LONG(lDrawTime[i]/60L));
	}
	...
	{
		_sntprintf(szBuffer,CountChar(szBuffer),TEXT("%I64d"),lStepTime[i]/60L);
		pComboBox->SetItemData(pComboBox->InsertString(i,szBuffer),LONG(lStepTime[i]/60L));
	}
	...
	{
		_sntprintf(szBuffer,CountChar(szBuffer),TEXT("%I64d"),lRuleSecondTime[i]);
		pComboBox->SetItemData(pComboBox->InsertString(i,szBuffer),LONG(lRuleSecondTime[i]));
	}
...
VOID CDlgRuleOption::OnOK()
{
	...
	if(m_pSinkWnd!=NULL)
	 m_pSinkWnd->SendEngineMessage(IDM_GAME_RULE_AGREE,0,0);
...
{
	if(m_pSinkWnd!=NULL)
		m_pSinkWnd->SendEngineMessage(IDM_GAME_RULE_REJECT,0,0);
...
bool CDlgRuleOption::SetComboBoxValue(UINT uComboBoxID, WORD wValue)
{
	...
	//构造字符
	TCHAR szBuffer[16];
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%d"),wValue);
...
VOID CDlgRuleOption::OnPaint()
{
	...
	if(pSkinResourceManager!=NULL)
		DCBuffer.SelectObject(pSkinResourceManager->GetDefaultFont());

	//绘画背景
	//CImageHandle HandleViewBack(&m_ImageViewBack);
	m_ImageViewBack.BitBlt(DCBuffer,0,0);
	//m_ImageViewBack.TransDrawImage(&DCBuffer,0,0,RGB(255,0,255));
	//绘画信息
	if (m_bControl==false)
	{
		//设置 DC
