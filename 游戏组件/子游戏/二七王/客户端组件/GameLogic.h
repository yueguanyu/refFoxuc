...
//特殊牌
//#define SPECIAL_CARD				0x25								//红五

//牌副定义
#define MAX_PACK					4									//最大副数
...
struct tagAnalyseResult
{
	...
	tagSameDataInfo					SameDataInfo[MAX_PACK];				//同牌数据
	tagTractorDataInfo				TractorDataInfo[MAX_PACK-1];		//拖牌数据
};

//需求结构
struct tagDemandInfo
{
	BYTE 							cbMaxTractor[MAX_PACK-1];			//长度需求
	BYTE							cbSameCardCount[MAX_PACK];			//同牌需求
	BYTE 							cbTractorCardCount[MAX_PACK-1];		//拖牌需求
};
...
public:
	...
	//设置甩牌
	void SetCanThrow(bool bCanThrow) { m_bCanThrow=bCanThrow; return;}
	//信息函数
public:
	//主牌花色
