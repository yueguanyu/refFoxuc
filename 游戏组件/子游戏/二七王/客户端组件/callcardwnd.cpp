...
{
	m_ImageBack.LoadFromResource(AfxGetInstanceHandle(),IDB_COLOR_BACK);
}
	...
	case IDC_CALL_NT:			//无主
		{
			...
			if(m_pSinkWindow!=NULL)
				m_pSinkWindow->PostEngineMessage(IDM_CALL_CARD,COLOR_NT,COLOR_NT);
	...
	case IDC_CALL_HEI_TAO:		//黑桃
		{
			//发送消息
			if(m_pSinkWindow!=NULL)
				m_pSinkWindow->PostEngineMessage(IDM_CALL_CARD,COLOR_HEI_TAO,COLOR_HEI_TAO);
	...
	case IDC_CALL_HONG_TAO:		//红桃
		{
			//发送消息
			if(m_pSinkWindow!=NULL)
				m_pSinkWindow->PostEngineMessage(IDM_CALL_CARD,COLOR_HONG_TAO,COLOR_HONG_TAO);
	...
	case IDC_CALL_MEI_HUA:		//梅花
		{
			//发送消息
			if(m_pSinkWindow!=NULL)
				m_pSinkWindow->PostEngineMessage(IDM_CALL_CARD,COLOR_MEI_HUA,COLOR_MEI_HUA);
	...
	case IDC_CALL_FANG_KUAI:	//方块
		{
			//发送消息
			if(m_pSinkWindow!=NULL)
				m_pSinkWindow->PostEngineMessage(IDM_CALL_CARD,COLOR_FANG_KUAI,COLOR_FANG_KUAI);
...
int CCallCardWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btCallNT.SetButtonImage(IDB_CALL_NT,hInstance,false,false);
	m_btCallHeiTao.SetButtonImage(IDB_CALL_HEI_TAO,hInstance,false,false);
	m_btCallHongTao.SetButtonImage(IDB_CALL_HONG_TAO,hInstance,false,false);
	m_btCallMeiHua.SetButtonImage(IDB_CALL_MEI_HUA,hInstance,false,false);
	m_btCallFangKuai.SetButtonImage(IDB_CALL_FANG_KUAI,hInstance,false,false);

	//获取位置
	CRect rcButton;
	m_btCallHeiTao.GetClientRect(&rcButton);
	...
	m_btCallHeiTao.SetWindowPos(NULL,	nXpos+(nSpace+rcButton.Width())*0		,nYpos			,0,0,SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOSIZE);
	m_btCallHongTao.SetWindowPos(NULL,	nXpos+(nSpace+rcButton.Width())*1		,nYpos			,0,0,SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOSIZE);
	m_btCallMeiHua.SetWindowPos(NULL,	nXpos+(nSpace+rcButton.Width())*2		,nYpos			,0,0,SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOSIZE);
	m_btCallFangKuai.SetWindowPos(NULL,	nXpos+(nSpace+rcButton.Width())*3		,nYpos			,0,0,SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOSIZE);

	m_btCallNT.SetWindowPos(NULL,		0						,0			,0,0,SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOSIZE);
	m_btCallNT.ShowWindow(SW_HIDE);

	//移动窗口
	SetWindowPos(NULL,0,0,m_ImageBack.GetWidth(),m_ImageBack.GetHeight(),SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS);
