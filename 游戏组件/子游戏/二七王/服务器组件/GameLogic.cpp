...
{
	//效验参数
	ASSERT((cbPackCount>0)&&(cbPackCount<=MAX_PACK));
	if ((cbPackCount==0)||(cbPackCount>MAX_PACK)) return false;
	...
	if(m_cbPackCount==3) cbConcealCount=10;
...
BYTE CGameLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if(m_bCanThrow==false) return CT_ERROR;
	if(m_bCanThrow==true&&GetCardLogicColor(cbCardData,cbCardCount)==COLOR_NT) return CT_THROW_CARD;
	return CT_ERROR;
}
...
{
	//混乱准备
	BYTE cbCardData[CELL_PACK*MAX_PACK];
	for (BYTE i=0;i<MAX_PACK;i++) CopyMemory(&cbCardData[CELL_PACK*i],m_cbCardData,sizeof(m_cbCardData));
...
bool CGameLogic::EfficacyThrowCard(const BYTE cbOutCardData[], BYTE cbOutCardCount, WORD wOutCardIndex, const BYTE cbHandCardData[GAME_PLAYER][MAX_COUNT], BYTE cbHandCardCount, tagOutCardResult & OutCardResult)
{
	...
	//	//比较大小
	//	if(GetCardLogicValue(cbDistillCardHand[0])>GetCardLogicValue(cbOutCardData[cbOutCardCount-1]))
	//	{
	//		OutCardResult.cbCardCount=1;
	//		OutCardResult.cbResultCard[0]=cbOutCardData[cbOutCardCount-1];
	//		return false;
	//	}
	//}
...
BYTE CGameLogic::GetCardSortOrder(BYTE cbCardData)
{
	...
	{
		BYTE cbSortValue=cbCardValue+14;
	...
	if (cbCardValue==7)
	{
		BYTE cbLogicValue=cbCardValue+14;
		BYTE cbDecreaseOrder=(m_cbMainColor==cbCardColor)?1:0;
	...
	if (cbCardValue==2)
	{
		BYTE cbLogicValue=cbCardValue+13;
		BYTE cbDecreaseOrder=(m_cbMainColor==cbCardColor)?1:0;
		return cbLogicValue+m_cbSortRight[4]+cbDecreaseOrder;		
	}
...
BYTE CGameLogic::GetCardLogicValue(BYTE cbCardData)
{
	...
	if ((cbCardData==0x4E)||(cbCardData==0x4F))
	{
		BYTE cbLogicValue=cbCardValue+15+cbNTRight;
	...
	if (cbCardValue==7)
	{
		BYTE cbLogicValue=cbCardValue+14+cbNTRight;
		BYTE cbDecreaseOrder=(cbCardColor==m_cbMainColor)?1:0;
		return cbLogicValue+m_cbSortRight[4]+cbDecreaseOrder;		
	}
	...
	if (cbCardValue==2)
	{
		BYTE cbLogicValue=cbCardValue+13+cbNTRight;
		BYTE cbDecreaseOrder=(cbCardColor==m_cbMainColor)?1:0;
		return cbLogicValue+m_cbSortRight[4]+cbDecreaseOrder;		
	}
...
BYTE CGameLogic::GetCardLogicColor(BYTE cbCardData)
{
	...
	//特殊判断
	if(cbCardValue==7 || cbCardValue==2) return COLOR_NT;
...
bool CGameLogic::IsLineValue(BYTE cbFirstCard, BYTE cbSecondCard)
{
	...
	BYTE bFirstValue=GetCardValue(cbFirstCard);
	BYTE bFirstColor=GetCardColor(cbFirstCard);
	...
	if( bFirstValue==0x0E && bNextValue==7 && bNextColor==m_cbMainColor) bIsLine=true;
	if( bFirstValue==7 && bFirstColor==m_cbMainColor && bNextValue==7 && bNextColor!=m_cbMainColor) bIsLine=true;
	if( bFirstValue==7 && bFirstColor!=m_cbMainColor && bNextValue==2 && bNextColor==m_cbMainColor) bIsLine=true;
	if( bFirstValue==2 && bFirstColor==m_cbMainColor && bNextValue==2 && bNextColor!=m_cbMainColor) bIsLine=true;
	if( bFirstValue==2 && bFirstColor!=m_cbMainColor && bNextValue==1 && bNextColor==m_cbMainColor) bIsLine=true;
	if( bFirstValue==8  && bNextValue==6 ) bIsLine=true;
