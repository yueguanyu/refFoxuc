...
void CGameClientView::RectifyControl(int nWidth, int nHeight)
{
	...
	for (int i = 0;i<8;i++)
	{
		if(i<4)
		{
			m_RectArea[i].top =nHeight/2-151;
	...
	//其他按钮
	//其他按钮
	DeferWindowPos(hDwp,m_btBankStorage,NULL,LifeWidth+10-1+571,TopHeight+592-12+33,104,400,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBankDraw,NULL,LifeWidth+10-1+571,TopHeight+592+50-17+33,0,0,uFlags|SWP_NOSIZE);
	//结束移动
...
void CGameClientView::DrawBankerInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	{
		_sntprintf(Text,256,_TEXT("当前庄家：%s"),pUserData==NULL?(m_bEnableSysBanker?TEXT("系统坐庄"):TEXT("无人坐庄")):pUserData->GetNickName());
		pDC->DrawText(Text, StrRect, DT_END_ELLIPSIS | DT_LEFT | DT_TOP|DT_SINGLELINE );
	}
	else
	{
		_sntprintf(Text,256,_TEXT("当前庄家：%s"),pUserData==NULL?(m_bEnableSysBanker==false?TEXT("无人坐庄"):TEXT("系统坐庄")):pUserData->GetNickName());
		pDC->DrawText(Text, StrRect, DT_END_ELLIPSIS | DT_LEFT | DT_TOP|DT_SINGLELINE );
	}
...
void CGameClientView::DrawMeInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	//庄家名字
	TCHAR Text[256];
	_sntprintf(Text,256,_TEXT("昵称：%s"),pUserData->GetNickName());
	pDC->DrawText(Text, StrRect, DT_END_ELLIPSIS | DT_LEFT | DT_TOP|DT_SINGLELINE );

	StrRect.top = StrRect.bottom;
	StrRect.bottom = StrRect.top + 15;
	_sntprintf(Text,256,_TEXT("金币：%s"),NumberStringWithSpace(pUserData->GetUserScore()));
	pDC->DrawText(Text, StrRect, DT_END_ELLIPSIS | DT_LEFT | DT_TOP|DT_SINGLELINE );
	...
	_sntprintf(Text,256,_TEXT("可用：%s"),NumberStringWithSpace(pUserData==NULL?0:pUserData->GetUserScore()-lMeJetton));
	pDC->DrawText(Text, StrRect, DT_END_ELLIPSIS | DT_LEFT | DT_TOP|DT_SINGLELINE );
