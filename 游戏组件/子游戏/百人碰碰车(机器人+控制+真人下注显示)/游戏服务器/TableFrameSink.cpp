	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			/*CString s;
			s.Format(_T("发送场景 bSuccess：%d"),bSuccess);
			AfxMessageBox(s);*/
			//限制提示
			TCHAR szTipMsg[128];
	...
	case GS_GAME_END:			//结束状态
		{
			...
			/*CString s;
			s.Format(_T("发送场景 12234 bSuccess：%d"),bSuccess);
			AfxMessageBox(s);*/
...
bool CTableFrameSink::OnUserPlaceJetton(WORD wChairID, BYTE cbJettonArea, LONGLONG lJettonScore)
{
		...
		if(pIServerUserItem->IsAndroidUser())
		{
			...
			if (!bHaveChip)
			{
				if (m_nChipRobotCount+1 > m_nMaxChipRobot)
				{
					...
					//SendGameMessage(INVALID_CHAIR,TEXT("下注失败 1"));
				}
				else
					m_nChipRobotCount++;
			}
	...
	{
		bPlaceJettonSuccess=false;
		//SendGameMessage(INVALID_CHAIR,TEXT("下注失败 2"));
	}
	...
	if (bPlaceJettonSuccess)
	{
		...
		//CString s;
		//s.Format(_T("下注成功,%I64d"),PlaceJetton.lJettonScore);
		//SendGameMessage(INVALID_CHAIR,s);
		//
		//发送消息
...
bool CTableFrameSink::OnUserApplyBanker(IServerUserItem *pIApplyServerUserItem)
{
	...
	if (lUserScore<m_lApplyBankerCondition)
	{
			//提示消息
			TCHAR szTipMsg[128] = {};
			_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("由于你的金币数（%I64d）少于坐庄必须金币数（%I64d）,申请失败！"),
		lUserScore, m_lApplyBankerCondition);
		m_pITableFrame->SendGameMessage(pIApplyServerUserItem,szTipMsg,SMT_CHAT|SMT_EJECT);
		return true;
	}
