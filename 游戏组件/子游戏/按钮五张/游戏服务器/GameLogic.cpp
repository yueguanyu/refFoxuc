	...
	case CT_TONG_HUA_SHUN:	//同花顺
		{
			...
			if( cbFirstType != CT_TONG_HUA )
			{
				...
				if( GetCardValue(cbNextData[0])==1 &&
					GetCardLogicValue(cbNextData[0])-cbMinusCount == GetCardLogicValue(cbNextData[1]) )
					cbNextValue = GetCardLogicValue(cbNextData[1]);
				if( GetCardValue(cbFirstData[0])==1 &&
					GetCardLogicValue(cbFirstData[0])-cbMinusCount == GetCardLogicValue(cbFirstData[1]) )
					cbFirstValue = GetCardLogicValue(cbFirstData[1]);
			}
#endif
