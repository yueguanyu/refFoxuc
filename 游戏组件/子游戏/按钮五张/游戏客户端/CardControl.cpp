...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	if( !m_bMoving || m_nXDistance > 0 )
	{
		for (WORD i=0;i<m_wCardCount;i++)
		{
			...
			//绘画扑克
			m_ImageCard.BlendDrawImage(pDC,OriginPoint.x+m_nXDistance*i,OriginPoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255),255);
		}
	}
	else
	{
		...
		m_ImageCard.BlendDrawImage( pDC,OriginPoint.x,OriginPoint.y-nYMove,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255),255);
		nXImagePos=m_CardSize.cx*2;
		nYImagePos=m_CardSize.cy*4;
		m_ImageCard.BlendDrawImage( pDC,OriginPoint.x,OriginPoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255),255);
	}
