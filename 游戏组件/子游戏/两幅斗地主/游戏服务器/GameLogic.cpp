...
void CGameLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount, BYTE cbSortType)
{
	...
	else if (cbSortType==ST_COUNT)
	{
		...
		//拷贝八牌
		CopyMemory(&cbCardData[cbIndex],AnalyseResult.cbEightCardData,sizeof(BYTE)*AnalyseResult.cbEightCount*8);
		cbIndex+=AnalyseResult.cbEightCount*8;

		//拷贝七牌
		CopyMemory(&cbCardData[cbIndex],AnalyseResult.cbSevenCardData,sizeof(BYTE)*AnalyseResult.cbSevenCount*7);
		cbIndex+=AnalyseResult.cbSevenCount*7;

		//拷贝六牌
		CopyMemory(&cbCardData[cbIndex],AnalyseResult.cbSixCardData,sizeof(BYTE)*AnalyseResult.cbSixCount*6);
		cbIndex+=AnalyseResult.cbSixCount*6;

		//拷贝五牌
		CopyMemory(&cbCardData[cbIndex],AnalyseResult.cbFiveCardData,sizeof(BYTE)*AnalyseResult.cbFiveCount*5);
		cbIndex+=AnalyseResult.cbFiveCount*5;
...
void CGameLogic::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResult & OutCardResult) 
{
	...
	for(BYTE cbCardType=CT_SINGLE; cbCardType<=CT_MISSILE_CARD; ++cbCardType)
		...
		{
			for(LONGLONG lIndex=0; lIndex<CardTypeResult[cbCardType].cbCardTypeCount; ++lIndex)
			{
...
void CGameLogic::UpsideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult) 
{
	...
	if( m_wBankerUser != wOutCardUser && ! CompareCard( cbTurnCardData, m_cbAllCardData[ m_wBankerUser ], cbTurnCardCount, m_cbUserCardCount[ m_wBankerUser ] ) )
	{
		...
		for(BYTE cbCardType=CT_SINGLE; cbCardType<=CT_MISSILE_CARD; ++cbCardType)
			...
			{
				for(LONGLONG lIndex=0; lIndex<FriendCardTypeResult[cbCardType].cbCardTypeCount; ++lIndex)
				{
...
bool CGameLogic::MagicCardData(BYTE cbCardData[], BYTE cbCardCount, BYTE cbResultCard[MAX_COUNT], BYTE cbCardType)
{
	...
	for (BYTE i=0;i<cbCardCount;i++)
	{
		...
		//变幻统计
		if ((cbCardValue==0x0E)||(cbCardValue==0x0F)) cbMagicCardCount++;
	...
	//三带二型
	if ((cbCardCount%5==0)&&(cbMaxSameCount<=3))
	{
		//变量定义
		BYTE bTempMagicData[MAX_COUNT];ZeroMemory(bTempMagicData,sizeof(bTempMagicData));
		BYTE bTemp3Take2[MAX_COUNT];ZeroMemory(bTemp3Take2,sizeof(bTemp3Take2));

		//循环变化
		if(cbMagicCardCount==1)
		{
			BYTE bTempCard=0;
			for(bTempCard=13;bTempCard>0;bTempCard--)
			{
				CopyMemory(bTempMagicData,cbNormalCard,sizeof(BYTE)*cbNormalCardCount);
				bTempMagicData[cbNormalCardCount]=bTempCard;
				SortCardList(bTempMagicData,cbNormalCardCount+1,ST_ORDER);

				if(GetCardType(bTempMagicData,cbNormalCardCount+1)==CT_THREE_LINE_TAKE_TWO)
				{
					if(bTemp3Take2[0]==0) CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+1)*sizeof(BYTE));
					else if(CompareCard(bTemp3Take2,bTempMagicData,cbNormalCardCount+1,cbNormalCardCount+1)==true)
					CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+1)*sizeof(BYTE));
				}
			}
		}
		//变化成功
		if(bTemp3Take2[0]!=0) 
		{
			CopyMemory(cbResultCard,bTemp3Take2,(cbNormalCardCount+1)*sizeof(BYTE));
			return true;
		}
		if(cbMagicCardCount==2)
		{
			for(BYTE bTempCard1=13;bTempCard1>0;bTempCard1--)
			{
				for(BYTE bTempCard2=13;bTempCard2>0;bTempCard2--)
				{
					CopyMemory(bTempMagicData,cbNormalCard,sizeof(BYTE)*cbNormalCardCount);
					bTempMagicData[cbNormalCardCount]=bTempCard1;
					bTempMagicData[cbNormalCardCount+1]=bTempCard2;
					SortCardList(bTempMagicData,cbNormalCardCount+2,ST_ORDER);

					if(GetCardType(bTempMagicData,cbNormalCardCount+2)==CT_THREE_LINE_TAKE_TWO)
					{
						if(bTemp3Take2[0]==0) CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+2)*sizeof(BYTE));
						else if(CompareCard(bTemp3Take2,bTempMagicData,cbNormalCardCount+2,cbNormalCardCount+2)==true)
						CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+2)*sizeof(BYTE));
					}
				}
			}
		}
		...
		if(bTemp3Take2[0]!=0) 
		{
			CopyMemory(cbResultCard,bTemp3Take2,(cbNormalCardCount+2)*sizeof(BYTE));
			return true;
		}
		if(cbMagicCardCount==3)
		{
			for(BYTE bTempCard1=13;bTempCard1>0;bTempCard1--)
			{
				for(BYTE bTempCard2=13;bTempCard2>0;bTempCard2--)
				{
					for(BYTE bTempCard3=13;bTempCard3>0;bTempCard3--)
					{
						CopyMemory(bTempMagicData,cbNormalCard,sizeof(BYTE)*cbNormalCardCount);
						bTempMagicData[cbNormalCardCount]=bTempCard1;
						bTempMagicData[cbNormalCardCount+1]=bTempCard2;
						bTempMagicData[cbNormalCardCount+2]=bTempCard3;
						SortCardList(bTempMagicData,cbNormalCardCount+3,ST_ORDER);

						if(GetCardType(bTempMagicData,cbNormalCardCount+3)==CT_THREE_LINE_TAKE_TWO)
						{
							if(bTemp3Take2[0]==0) CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+3)*sizeof(BYTE));
							else if(CompareCard(bTemp3Take2,bTempMagicData,cbNormalCardCount+3,cbNormalCardCount+3)==true)
							CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+3)*sizeof(BYTE));
						}
					}
				}
			}
		...
		if(bTemp3Take2[0]!=0) 
		{
			CopyMemory(cbResultCard,bTemp3Take2,(cbNormalCardCount+3)*sizeof(BYTE));
			return true;
		}
		if(cbMagicCardCount==4)
		{
			for(BYTE bTempCard1=13;bTempCard1>0;bTempCard1--)
			{
				for(BYTE bTempCard2=13;bTempCard2>0;bTempCard2--)
				{
					for(BYTE bTempCard3=13;bTempCard3>0;bTempCard3--)
					{
						for(BYTE bTempCard4=13;bTempCard4>0;bTempCard4--)
						{
							CopyMemory(bTempMagicData,cbNormalCard,sizeof(BYTE)*cbNormalCardCount);
							bTempMagicData[cbNormalCardCount]=bTempCard1;
							bTempMagicData[cbNormalCardCount+1]=bTempCard2;
							bTempMagicData[cbNormalCardCount+2]=bTempCard3;
							bTempMagicData[cbNormalCardCount+3]=bTempCard4;
							SortCardList(bTempMagicData,cbNormalCardCount+4,ST_ORDER);

							if(GetCardType(bTempMagicData,cbNormalCardCount+4)==CT_THREE_LINE_TAKE_TWO)
							{
								if(bTemp3Take2[0]==0) CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+4)*sizeof(BYTE));
								else if(CompareCard(bTemp3Take2,bTempMagicData,cbNormalCardCount+4,cbNormalCardCount+4)==true)
								CopyMemory(bTemp3Take2,bTempMagicData,(cbNormalCardCount+4)*sizeof(BYTE));
							}
						}
					}
				}
		...
		if(bTemp3Take2[0]!=0) 
		{
			CopyMemory(cbResultCard,bTemp3Take2,(cbNormalCardCount+4)*sizeof(BYTE));
			return true;
		}
	}
	...
	if (((cbMaxSameCount==2))&&( (cbCardCount>=6)&&(cbCardCount%2==0) )) bMagicLink=true;
	if (((cbMaxSameCount==3))&&( (cbCardCount>=6)&&(cbCardCount%3==0) )) bMagicLink=true;
