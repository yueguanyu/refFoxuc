...
CScoreView::CScoreView()
{
	...
	ZeroMemory(m_lAddScore,sizeof(m_lAddScore));
...
void CScoreView::OnPaint() 
{
	...
	for (WORD i=0;i<CountArray(m_szUserName);i++)
	{
		if (m_szUserName[i][0]!=0)
		{
			...
			myprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),m_lAddScore[i]);
			dc.DrawText(szBuffer,lstrlen(szBuffer),&rcScore,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

			//输出成绩
			CRect rcScore1(164,wCount*29+nScPos,264,(wCount+1)*29+nScPos);
			myprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),m_lScore[i]);
			dc.DrawText(szBuffer,lstrlen(szBuffer),&rcScore1,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
			...
			myprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),m_lTax[i]);
...
//设置积分
void CScoreView::SetGameScore(WORD wChairID, LPCTSTR pszUserName, LONGLONG lScore,LONGLONG lAddScore)
{
	if (wChairID<CountArray(m_lScore))
	{
		m_lAddScore[wChairID]=lAddScore;
