...
#else
#define TIME_PLACE_JETTON			20//20									//下注时间
#endif
...
#else
#define TIME_GAME_END				20//20									//结束时间
#endif
...
//初始化各区域赔率
const int CTableFrameSink::m_iOddsArray[JETTON_AREA_COUNT]={50,20,20,20,10,10,10,5};//

//////////////////////////////////////////////////////////////////////////

//构造函数
CTableFrameSink::CTableFrameSink() : m_iMaxAreaScore(10000)
{
	//总下注数
	...
	GetPrivateProfileString(TEXT("BankerCondition"),TEXT("Score"),TEXT("50000"),szBuffer,CountArray(szBuffer),m_szConfigFileName);
	//m_iApplyBankerCondition=_atoi64(szBuffer);
	myscanf(szBuffer,mystrlen(szBuffer),TEXT("%I64d"),&m_iApplyBankerCondition);
	...
	myscanf(szBuffer,mystrlen(szBuffer),TEXT("%I64d"),&m_iSysLimitScore);

	GetPrivateProfileString(TEXT("ScoreLimit"),TEXT("UserLimitScore"),TEXT("10000"),szBuffer,CountArray(szBuffer),m_szConfigFileName);
	...
	myscanf(szBuffer,mystrlen(szBuffer),TEXT("%I64d"),&m_iUserLimitScore);
...
bool CTableFrameSink::OnEventGameStart()
{
	...
	if (pIBankerServerUserItem!=NULL) 
		GameStart.iBankerScore=pIBankerServerUserItem->GetUserScore();
	...
	for (WORD wChairID=0; wChairID<GAME_PLAYER; ++wChairID)
	{
		...
		//设置积分
		GameStart.iUserMaxScore=min(pIServerUserItem->GetUserScore(),m_iUserLimitScore);
...
bool __cdecl CTableFrameSink::CalculateAreaInitLimite()
{
	...
	if( INVALID_CHAIR != m_wCurrentBanker )
	{
		...
		//获得庄家当前的积分(金币)
		iBankerScore = pIBankerServerUserItem->GetUserScore();
	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			if (m_wCurrentBanker!=INVALID_CHAIR)
			{
				...
				StatusFree.iBankerScore=pIServerUserItem->GetUserScore();
			}

			//玩家信息
			if (pIServerUserItem->GetUserStatus()!=US_LOOKON)
			{
				StatusFree.iUserMaxScore=min(pIServerUserItem->GetUserScore(),m_iUserLimitScore); 
			}
	...
	case GS_GAME_END:			//结束状态
		{
			...
			if (pIServerUserItem->GetUserStatus()!=US_LOOKON)
			{
				...
				//最大下注
				StatusPlay.iUserMaxScore=min(pIServerUserItem->GetUserScore(),m_iUserLimitScore);
			}
			...
			{
				IServerUserItem *pIServerUserItem=m_pITableFrame->GetTableUserItem(m_wCurrentBanker);
				StatusPlay.iBankerScore=pIServerUserItem->GetUserScore();
			}	
	...
	case IDI_GAME_END:			//结束游戏
		{
			...
			//结束游戏
			m_pITableFrame->ConcludeGame(GER_NORMAL);//注意参数
	...
	if( m_bControlWin )
	{
		if( m_wCurrentBanker != INVALID_CHAIR )	//当前有人做庄
		{
			...
			for( int i=0; i< m_CtrlAccountArray.GetCount(); i++ )
			{
				...
				if( tempAccount.dwUserID == pIServerUserItem->GetUserInfo()->dwUserID )
				{
...
bool CTableFrameSink::ChangeBanker(bool bCancelCurrentBanker)
{
	...
	else if (m_wCurrentBanker!=INVALID_CHAIR)
	{
		...
		if (m_nMaxBankerTimes<=m_wBankerTime || iBankerScore<m_iApplyBankerCondition)
		{
			...
			if (iBankerScore<m_iApplyBankerCondition)
				_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("[ %s ]分数少于(%I64d)，强行换庄!"),pIServerUserItem->GetNickName(),m_iApplyBankerCondition);
			else
	...
	if (bChangeBanker)
	{
		...
		if (m_wCurrentBanker!=INVALID_CHAIR)
		{
			...
			if( pIServerUserItem == NULL )
				return FALSE;
			ChangeBanker.iBankerScore=pIServerUserItem->GetUserScore();
		}
		else
		{
			//如果if的条件不满足，应该如何处理？
...
__int64 CTableFrameSink::CalculateScore()
{
	...
	//中奖区域
	BYTE cbHotArea = m_iGoalAnimal;//m_iGoalAnimal % (JETTON_AREA_COUNT);
	switch(cbHotArea)
	{
	case 0:
	case 11:
...
bool CTableFrameSink::GetOneAreaWinOdds(int nAreaIndex, __int64 & iResult)
{
	...
	for( int j=0; j<GAME_PLAYER; j++)
	{
		...
		IServerUserItem * pUserItem = m_pITableFrame->GetTableUserItem(j);
		if( pUserItem == NULL )
			continue;
