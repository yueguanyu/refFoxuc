...
void CApplyUser::ShowApplayBankerUser(CDC * pDC, CRect * pSortNoRect, CRect * pNameRect, CRect * pScoreRect)
{
	...
	for( int i=m_nPos; i<m_nPos+nEnd; i++)
	{
		...
		pDC->DrawText(m_BankerUserArray[i].szUserName,lstrlen(m_BankerUserArray[i].szUserName), pNameRect[k++], DT_END_ELLIPSIS|DT_LEFT|DT_TOP|DT_SINGLELINE);
		strMsg.Format(TEXT("%I64d"), m_BankerUserArray[i].iUserScore);
		pDC->DrawText(strMsg, pScoreRect[m++], DT_END_ELLIPSIS|DT_LEFT|DT_TOP|DT_SINGLELINE);
	}
}
