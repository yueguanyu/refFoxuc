	...
	case GER_NORMAL:		//常规结束
		{
			...
			LONGLONG lWholeScore=GameEnd.wGameScore+GameEnd.wConcealTime*GameEnd.wConcealScore;

			//积分统计
			if (lWholeScore<160L)
			{
				//升级统计
				WORD wIndex=m_wBankerUser%2;
				LONGLONG lWinOrder=(159L-lWholeScore)/80L+1L;

				//得分计算
				lBankerScore=1<<lWinOrder;
				lBankerScore=__min(5000L,lBankerScore);

				//升级计算
				for (BYTE i=0;i<lWinOrder;i++)
				{
					m_cbValueOrder[wIndex]++;
					if (m_cbValueOrder[wIndex]==14) m_cbValueOrder[wIndex]=0x01;
				}
			}
			else
			{
				//升级统计
				WORD wIndex=(m_wBankerUser+1)%2;
				LONGLONG lWinOrder=(lWholeScore-160L)/80L;

				//得分计算
				lBankerScore=-(1<<lWinOrder);
				lBankerScore=__max(-5000L,lBankerScore);
			...
			{
				LONGLONG lRevence = m_pGameServiceOption->wRevenueRatio;
				for (WORD i=0;i<m_wPlayerCount;i++)
				{
					if (GameEnd.lScore[i]>=10L)
					{
						lGameTax[i]=GameEnd.lScore[i]*lRevence/1000L;
			...
			for( WORD i = 0; i < GAME_PLAYER; i++ )
			{
				...
				if( GameEnd.lScore[i] == 0 ) ScoreInfo[i].cbType = SCORE_TYPE_DRAW;
				else if( GameEnd.lScore[i] > 0 ) ScoreInfo[i].cbType = SCORE_TYPE_WIN;
	...
	case GER_USER_LEAVE:		//用户强退
		{
			...
			for( WORD i = 0; i < GAME_PLAYER; i++ )
			{
				...
				else
				{
					...
					//m_pITableFrame->WriteUserScore(i,GameEnd.lScore[i],0,enScoreKind_Draw);
				}
			}
			m_pITableFrame->WriteTableScore(ScoreInfo,CountArray(ScoreInfo));
			...
			myprintf(szMessage,CountArray(szMessage),TEXT("由于 [ %s ] 离开游戏，游戏结束"),pIServerUserItem->GetNickName());
			for (WORD i=0;i<m_wPlayerCount;i++)
			{
...
bool CTableFrameSink::OnUserRequestLeave(WORD wChairID, LPCTSTR pszLeaveReason)
{
	...
	if (m_wRequestTimes[wChairID]>=2)
	{
		...
		m_pITableFrame->SendGameMessage(pIServerUserItem,TEXT("您请求离开的次数太多了，请稍后再次尝试！"),SMT_CHAT);
	}
...
bool CTableFrameSink::OnUserResponsesLeave(WORD wChairID, WORD wTargerChairID, BYTE cbAgreeLeave)
{
	...
	{
		//获取用户
		IServerUserItem * pIResponsesUserItem=m_pITableFrame->GetTableUserItem(wChairID);
		IServerUserItem * pIRequestUserItem=m_pITableFrame->GetTableUserItem(wTargerChairID);

		//构造消息
		TCHAR szMessage[128]=TEXT("");
		myprintf(szMessage,CountArray(szMessage),TEXT("“%s”%s您离开！"),
			pIResponsesUserItem->GetNickName(),(cbAgreeLeave==TRUE)?TEXT("同意"):TEXT("不同意"));

		//发送消息
		m_pITableFrame->SendGameMessage(pIRequestUserItem,szMessage,SMT_CHAT);
