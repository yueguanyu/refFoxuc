...
void CScoreView::OnPaint() 
{
	...
	if (m_cbConcealCount>0)
	{
		...
		for (BYTE i=0;i<m_cbConcealCount;i++)
		{
			...
			//绘画扑克
			m_ImageCard.TransDrawImage(&dc,nXPos+i*18,nYPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
		}
	}
	...
	myprintf(szBuffer,CountArray(szBuffer),TEXT("游戏得分 %d + 底牌分 %d × %d 倍 = %d"),m_wGameScore,
	...
	for (WORD i=0;i<CountArray(m_szUserName);i++)
	{
		if (m_szUserName[i][0]!=0)
		{
			...
			m_ImageWinLose.TransDrawImage(&dc,261-nWinLoseWidth/2,i*29+239-nWinLostHeight/2,nWinLoseWidth,nWinLostHeight,
