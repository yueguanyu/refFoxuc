...
bool CGameClientEngine::OnInitGameEngine()
{
	...
	//AfxMessageBox(TEXT("CGameClientEngine::OnInitGameEngine"));
	...
	case IDI_SEND_CONCEAL:		//发送底牌
		{
			if ((nElapse==0)||((m_bStustee==true)&&(nElapse<(TIME_SEND_CONCEAL))))
			{
				...
				if ((IsLookonMode()==false)&&(wChairID==GetMeChairID()))
				{
					...
					if (m_bStustee==false) 
					{
						//InsertSystemString(TEXT("限定的时间到了，系统为您自动埋底牌"));
						if(m_pIStringMessage!=NULL)
...
//用户叫牌
bool CGameClientEngine::OnSubCallCard(VOID * pBuffer, WORD wDataSize)
{
	//效验数据
...
//发送底牌
bool CGameClientEngine::OnSubSendConceal(VOID * pBuffer, WORD wDataSize)
{
	//效验数据
...
//游戏开始
bool CGameClientEngine::OnSubGamePlay(VOID * pBuffer, WORD wDataSize)
{
	//效验数据
...
bool CGameClientEngine::OnSubOutCard(VOID * pBuffer, WORD wDataSize)
{
	...
	{
		//删除扑克 
		m_GameLogic.RemoveCard(pOutCard->cbCardData,pOutCard->cbCardCount,m_cbHandCardData,m_cbHandCardCount);
...
//甩牌结果
bool CGameClientEngine::OnSubThrowResult(VOID * pBuffer, WORD wDataSize)
{
	//变量定义
...
//一轮结算
bool CGameClientEngine::OnSubTurnBalance(VOID * pBuffer, WORD wDataSize)
{
	//效验数据
...
bool CGameClientEngine::OnSubGameEnd(VOID * pBuffer, WORD wDataSize)
{
	...
	if(m_pIStringMessage!=NULL)
		m_pIStringMessage->InsertCustomString(TEXT("本局结束,成绩统计:"),RGB(255,0,255),RGB(255,255,255));

	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		IClientUserItem * pUserData=GetTableUserItem(i);
		m_GameClientView.m_ScoreView.SetGameScore(i,pUserData->GetNickName(),pGameEnd->lScore[i]);
		//在聊天框显示成绩
		myprintf(szBuffer,CountArray(szBuffer),TEXT(" %s: %+I64d分"),pUserData->GetNickName(),pGameEnd->lScore[i]);
		//InsertGeneralString(szBuffer,RGB(255,0,255),true);
		if(m_pIStringMessage!=NULL)
			m_pIStringMessage->InsertCustomString(szBuffer,RGB(255,0,255),RGB(255,255,255));
	}
...
//停止发牌
void CGameClientEngine::StopDispatchCard()
{
	if( m_cbRemnantCardCount > 0 )
	{
		//删除定时器
...
LRESULT CGameClientEngine::OnRuquestLeave(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		//发送消息
		SendSocketData(SUB_C_REQUEST_LEAVE,&RequestLeave,sizeof(RequestLeave));
		...
		if(m_pIStringMessage!=NULL)
			m_pIStringMessage->InsertSystemString(TEXT("离开请求已经发送，请耐心等待其他玩家回应。"));
	}
	return 0;
}
...
LRESULT CGameClientEngine::OnStusteeControl(WPARAM wParam, LPARAM lParam)
{
	...
	{
		TCHAR szBuffer[32];
		myprintf(szBuffer,CountArray(szBuffer),TEXT("您%s了托管功能."),m_bStustee?TEXT("选择"):TEXT("取消"));
	...
	SendSocketData(SUB_C_STRUSTEE,&Strustee,sizeof(Strustee));
...
//用户托管
bool CGameClientEngine::OnSubStrustee(VOID *pBuffer,WORD wDataSize)
{
	//变量定义
	...
	{
		IClientUserItem *pUserData = GetTableUserItem(pStrustee->wChairId);
		if( pUserData == NULL ) return true;
		TCHAR szBuffer[128] = TEXT("");
		myprintf(szBuffer,CountArray(szBuffer),TEXT("玩家[ %s ]%s了托管功能"),pUserData->GetNickName(),pStrustee->bStrustee?TEXT("选择"):TEXT("取消"));
		//InsertSystemString(szBuffer);
		if(m_pIStringMessage!=NULL)
			m_pIStringMessage->InsertSystemString(szBuffer);
	}
...
//请求离开
bool CGameClientEngine::OnSubRequestLeave( VOID *pBuffer,WORD wDataSize )
{
	//效验数据
	...
	//获取用户
	IClientUserItem * pUserItem=GetTableUserItem(pRequestLeave->wChairID);
	...
	if (pUserItem!=NULL&&!IsLookonMode())
	{
		...
		myprintf(szMessage,CountArray(szMessage),TEXT("玩家【%s】申请离开游戏，您是否同意结束游戏？附言："),
			(LPCTSTR)pUserItem->GetNickName());
...
//请求离开响应
LRESULT CGameClientEngine::OnMessageResponseLeave(WPARAM wParam,LPARAM lParam)
{
	...
	//发送消息
	SendSocketData(SUB_C_RESPONSES_LEAVE,&ResponsesLeave,sizeof(ResponsesLeave));
...
//批量出牌
bool CGameClientEngine::OnSubBatchCard(VOID *pBuffer,WORD wDataSize)
{
	//变量定义
...
//test
LRESULT CGameClientEngine::OnTest( WPARAM wParam,LPARAM lParam )
{
#ifdef TEST
	//发送数据
	CMD_C_OutCard OutCard;
