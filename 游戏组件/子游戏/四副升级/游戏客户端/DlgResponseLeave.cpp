...
BEGIN_MESSAGE_MAP(CDlgResponseLeave, CSkinDialog)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
//构造函数
CDlgResponseLeave::CDlgResponseLeave(CWnd * pParentWnd):CSkinDialog(IDD_REQUEST_LEAVE,pParentWnd)
{
...
VOID CDlgResponseLeave::OnOK()
{
	...
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_RESPONSE_LEAVE,(WPARAM)this,(LPARAM)TRUE);
	//关闭窗口
	DestroyWindow();
	return;
}
...
VOID CDlgResponseLeave::OnCancel()
{
	...
	CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
	if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_RESPONSE_LEAVE,(WPARAM)this,(LPARAM)FALSE);
	//关闭窗口
	DestroyWindow();
	return;
}
	...
	case IDI_DESTROY_DLG:
		{
			...
			{
				TCHAR szBuffer[32];
				myprintf(szBuffer,CountArray(szBuffer),TEXT("%s(%d)"),m_strCaption,m_wTimer);
			...
			CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
			if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_RESPONSE_LEAVE
				,(WPARAM)this,(LPARAM)FALSE);
			//关闭对话框
