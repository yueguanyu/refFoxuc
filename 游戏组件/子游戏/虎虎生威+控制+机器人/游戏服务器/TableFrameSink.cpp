...
bool  CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	myprintf(m_szConfigFileName,sizeof(m_szConfigFileName),TEXT("%s\\ZodiacBattle.ini"),szPath);
	...
	case SUB_C_ADMIN_COMMDN:
		{
			...
			if (pData->cbControlAppType == C_CA_CHEAK)
			{
				for ( WORD wUserChairID = 0; wUserChairID < GAME_PLAYER; ++wUserChairID )
				{
					IServerUserItem *pIServerUserItem = m_pITableFrame->GetTableUserItem(wUserChairID);
					if ( pIServerUserItem == NULL ) continue;
					if (lstrcmp(pIServerUserItem->GetNickName(),pData->dwUserAccount)==0)
					{
						CMD_S_UserJettonScore UserJettonScore;
						ZeroMemory(&UserJettonScore, sizeof(UserJettonScore));
						for (int nAreaIndex=1; nAreaIndex<=AREA_COUNT; ++nAreaIndex)
							UserJettonScore.lUserJettonScore[nAreaIndex] = m_lUserJettonScore[nAreaIndex][pIServerUserItem->GetChairID()];
