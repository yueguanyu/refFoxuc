...
bool CGameLogic::CompareMyCard(WORD MeChairID,WORD ChairID)
{
	if(CompareCard(m_cbAllCardData[MeChairID],m_cbAllCardData[ChairID],3)!=0) return true;
...
//设置扑克
VOID CGameLogic::SetUserCard(WORD wChairID, BYTE cbCardData[])
{
	CopyMemory(m_cbAllCardData[wChairID], cbCardData, 3*sizeof(BYTE)) ;

	//排列扑克
	SortCardList(m_cbAllCardData[wChairID], 3) ;
}
