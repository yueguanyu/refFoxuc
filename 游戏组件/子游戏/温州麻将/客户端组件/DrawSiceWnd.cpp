	...
	if(IDI_SICE_POS==nIDEvent)
	{
		...
		else
		{
			if(m_bSicePos==8)
			{
				...
				CGameFrameView *pWnd=CGameFrameView::GetInstance();
				if(pWnd)
					pWnd->PostEngineMessage(IDM_SICE_TWO,NULL,NULL);
			}
			m_bSicePos++;
		}
		Invalidate();
...
void CDrawSiceWnd::StopSicing(bool bFinish)
{
	if( IsWindowVisible() )
	{
		...
		if( m_bSicePos <= 8 && bFinish)
		{
			CGameFrameView *pWnd=CGameFrameView::GetInstance();
			if(pWnd)
				pWnd->PostEngineMessage( IDM_SICE_TWO,0,0 );
		}
		...
		if(bFinish)
		{
			CGameFrameView *pWnd=CGameFrameView::GetInstance();
			if(pWnd)
				pWnd->PostEngineMessage(IDM_SICE_FINISH,0,0);
		}
...
void CDrawSiceWnd::SetSiceInfo(CDC *dc,BYTE bMoveTime,WORD wSice1,WORD wSice2)
{
	HINSTANCE hInstance=AfxGetInstanceHandle();
	ParentDC=dc;
	//m_ImageBack.LoadFromResource(IDB_SICE_BACK,hInstance);
