...
{
	if (m_cbMagicIndex >= MAX_INDEX)	return 0;
	if (m_cbMagicIndex >= 0 && m_cbMagicIndex < MAX_INDEX-MAX_HUA_CARD)	return cbCardIndex[m_cbMagicIndex];
				...
				if( GetCardCount(cbCardIndexTemp) == 2 )
				{
					...
					{	//091231JJ 胡牌牌眼判断
						if (cbCardIndexTemp[i]==2)
						{
							...
							if( m_cbMagicIndex != MAX_INDEX && IsMagicCard(i, true) ) bMagicEye = true;
							break;
						}
...
bool CGameLogic::IsTuoPai(BYTE cbOutCard, tagWeaveItem * pWeaveItem)
{
	...
	if (cbOutCard == pWeaveItem->cbCenterCard)	return true;
...
bool CGameLogic::IsBaDui(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], const BYTE cbWeaveCount, bool & bYingBaDui)
{
	...
	for (BYTE i=0;i<MAX_INDEX;i++)
	{
		...
		//单牌统计
		if ( cbCardCount == 2 || cbCardCount == 3 ) 	cbDoubleCount++;
	}
...
{
	//无财神不算
	if (m_cbMagicIndex == MAX_INDEX)	return false;
