	...
	case GER_NORMAL:		//常规结束
		{
			...
			if (wChairID!=INVALID_CHAIR)
			{
				...
				ScoreInfo[wWinUser].cbType=SCORE_TYPE_WIN;
				ScoreInfo[wWinUser].lRevenue=0;
				ScoreInfo[wWinUser].lScore=GameEnd.lUserScore[wWinUser];

				ScoreInfo[wLostUser].cbType=SCORE_TYPE_LOSE;
				ScoreInfo[wLostUser].lRevenue=0;
				ScoreInfo[wLostUser].lScore=GameEnd.lUserScore[wLostUser];
			...
			else	
			{
				...
				ScoreInfo[0].cbType=SCORE_TYPE_DRAW;
				ScoreInfo[0].lRevenue=0;
				ScoreInfo[0].lScore=GameEnd.lUserScore[0];

				ScoreInfo[1].cbType=SCORE_TYPE_DRAW;
				ScoreInfo[1].lRevenue=0;
...
//发送场景
bool  CTableFrameSink::OnEventSendGameScene(WORD wChiarID, IServerUserItem * pIServerUserItem, BYTE cbGameStation, bool bSendSecret)
{
	...
	case GAME_STATUS_PLAY:	//游戏状态
		{
			...
			for (INT_PTR i=0;i<m_GameLogic.m_ChessManualInfo.GetCount();i++)
			{
				...
				{
					m_pITableFrame->SendUserItemData(pIServerUserItem,SUB_S_CHESS_MANUAL,cbBuffer,wBufferSize);
					wBufferSize=0;
				}
				CopyMemory(cbBuffer+wBufferSize,&m_GameLogic.m_ChessManualInfo[i],sizeof(tagChessManual));
				wBufferSize+=sizeof(tagChessManual);
			}
			if (wBufferSize>0) m_pITableFrame->SendUserItemData(pIServerUserItem,SUB_S_CHESS_MANUAL,cbBuffer,wBufferSize);
...
bool CTableFrameSink::OnUserPlaceChess(WORD wChairID, BYTE cbXPos, BYTE cbYPos)
{
	...
	//结束游戏
	if (bGameEnd==true) OnEventGameConclude(wWinUser,NULL,GER_NORMAL);
...
bool CTableFrameSink::OnUserTradeReq(WORD wChairID)
{
	...
	//玩家判断
	if (wChairID==m_wBlackUser) return true;
	if (m_wCurrentUser!=wChairID) return false;
