	...
	case GAME_STATUS_PLAY:	//游戏状态
		{
			...
			if (IsLookonMode()==false)
			{
				...
				//if ((m_bTradeUser==true)&&(wMeChairID!=m_wBlackUser)&&(cbAllStepCount==3))m_GameClientView.m_btTrade.EnableWindow(TRUE);
...
//交换对家
bool CGameClientEngine::OnSubBlackTrade(const void * pBuffer, WORD wDataSize)
{
	//对换用户
...
bool CGameClientEngine::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		IClientUserItem * pUserData=GetTableUserItem(wUserChairID);
	...
	if(m_pIStringMessage!=NULL)
	{
		...
		for (WORD i=0;i<GAME_PLAYER;i++)
		{
			...
			{
				_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%s:%+I64d"),pClientUserItem->GetNickName(),pGameEnd->lUserScore[i]);
				m_pIStringMessage->InsertCustomString(szBuffer,RGB(0,0,0)/*RGB(0,128,255)*/,RGB(255,255,255));
			}
			/*else if(m_szAccounts[i][0]=='#')
			{
				_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%s:%+I64d"),TEXT("用户离开"),pGameEnd->lUserScore[i]);
...
//手数按钮
LRESULT CGameClientEngine::OnShowChessID(WPARAM wParam, LPARAM lParam)
{
	//设置控件
...
//对换按钮
LRESULT CGameClientEngine::OnTrade(WPARAM wParam, LPARAM lParam)
{
	//交换确认
	...
	//发送消息
	SendSocketData(SUB_C_TRADE_REQ);
