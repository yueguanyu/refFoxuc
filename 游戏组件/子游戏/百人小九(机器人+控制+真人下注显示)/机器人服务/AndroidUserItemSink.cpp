...
//时间标识
#define IDI_PLACE_JETTON1			1									//下注定时
#define IDI_PLACE_JETTON2			2									//下注定时
#define IDI_PLACE_JETTON3			3									//下注定时
#define IDI_PLACE_JETTON4			4									//下注定时
#define IDI_PLACE_JETTON5			5									//下注定时
#define IDI_CHECK_BANKER			6									//检查上庄
#define IDI_REQUEST_BANKER			7									//申请定时
#define IDI_GIVEUP_BANKER			8									//下庄定时
#define IDI_PLACE_JETTON			9									//下注定义 (预留110-160)
	...
	case IDI_CHECK_BANKER:		//检查上庄
		{
			...
			if (m_wCurrentBanker == INVALID_CHAIR || m_stlApplyBanker < 4)
			{
				if(m_bMeApplyBanker)return true;
				//空庄
				m_nWaitBanker++;
...
bool CAndroidUserItemSink::OnSubGameFree(const void * pBuffer, WORD wDataSize)
{
	...
	m_stlApplyBanker = pGameFree->wBankCount;
...
void CAndroidUserItemSink::ReadConfigInformation(TCHAR szFileName[], TCHAR szRoomName[], bool bReadFresh)
{
	...
	_sntscanf(OutBuf, sizeof(OutBuf), _T("%I64d"), &m_lRobotJettonLimit[1]);

	ZeroMemory(OutBuf, sizeof(OutBuf));
	GetPrivateProfileString(szRoomName, TEXT("RobotMinJetton"), _T("100"), OutBuf, 255, szConfigFileName);
	_sntscanf(OutBuf, sizeof(OutBuf), _T("%I64d"), &m_lRobotJettonLimit[0]);
