...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_TotoalPress.n_Png_tatoalflag.LoadImage(this,hInstance,TEXT("IDB_IDB_TIPP_PNG"));
	m_TotoalPress.n_Png_tipC.LoadImage(this,hInstance,TEXT("IDB_IDB_TIPC_PNG"));
	m_TotoalPress.n_Png_tipN.LoadImage(this,hInstance,TEXT("IDB_IDB_TIPN_PNG"));
	m_PngScoreNumber.LoadImage(this,hInstance,TEXT("IDB_IDB_TIPN_PNG"));
	m_ImageTimer.LoadImage(this,hInstance,TEXT("IDB_IDB_CLOCK_PNG"));
	...
	m_btContinueCard.SetButtonImage(IDB_BT_CONTINUE_CARD,hResInstance,false,true);

	m_btBankerStorage.SetButtonImage(IDB_BT_STORAGE,hResInstance,false,true);
	m_btBankerDraw.SetButtonImage(IDB_BT_DRAW,hResInstance,false,true);
...
void CGameClientView::OnTimer(UINT nIDEvent)
{
	...
	{
		if(m_pGameClientEngine->GetGameStatus() != GS_GAME_END)
		{
			m_cbAreaFlash = 0xFF;
			KillTimer(IDI_FLASH_WINNER);
		}
	...
	if(IDI_OPENCARD==nIDEvent)
	{
		...
		//自动搓牌
		{
			...
			if(m_CardControl[m_OpenCardIndex%4].m_MovePoint.x<50&&m_CardControl[m_OpenCardIndex%4].m_blAdd)
			{
				...
				m_CardControl[m_OpenCardIndex%4].m_MovePoint.x+=3;
				m_CardControl[m_OpenCardIndex%4].m_MovePoint.y+=7;
...
{
	//显示判断
	//if (m_pGameClientEngine->GetGameStatus()!=GS_GAME_END) return;
...
void CGameClientView::DrawBankerInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	CDFontEx::DrawText(this,pDC,  12, 400, pUserData==NULL?(m_bEnableSysBanker?TEXT("系统坐庄"):TEXT("无人坐庄")):pUserData->GetNickName(), StrRect, m_CurSelectColor, DT_END_ELLIPSIS|DT_LEFT|DT_TOP|DT_SINGLELINE);
