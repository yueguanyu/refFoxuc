...
void CControlWnd::OnPaint()
{
	...
	for (BYTE i=0;i<CountArray(m_cbGangCard);i++)
	{
		if (m_cbGangCard[i]!=0)
		{
			...
			for (BYTE j=0;j<4;j++)
			{
				g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,m_cbGangCard[i],j*30+12,nYPos+5);
			}
	...
	if (m_cbCurrentItem!=0xFF)
	{
		...
		for (BYTE i=0;i<CountArray(wItemKind);i++)
		{
			if (((m_wActionMask&wItemKind[i])!=0)&&(m_cbCurrentItem==cbIndex++))
			{
				...
				CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
				if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_CARD_OPERATE,wItemKind[i],m_cbCenterCard);
				return;
			}
		}
		...
		for (BYTE i=0;i<CountArray(m_cbGangCard);i++)
		{
			if ((m_cbGangCard[i]!=0)&&(m_cbCurrentItem==cbIndex++))
			{
				...
				CGameFrameEngine * pGameFrameEngine=CGameFrameEngine::GetInstance();
				if (pGameFrameEngine!=NULL) pGameFrameEngine->PostMessage(IDM_CARD_OPERATE,WIK_GANG,m_cbGangCard[i]);
				return;
			}
		}
