...
bool CGameLogic::AnalyseCard(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount, CAnalyseItemArray & AnalyseItemArray)
{
	...
	if (cbLessKindItem==0)
	{
		...
		if (cbCardIndex[m_cbKingCardIndex[0]]==1&& cbCardIndex[m_cbKingCardIndex[1]]==1) 
		{
		 ...
		 //精牌+单张
		if ((cbCardIndex[m_cbKingCardIndex[0]]  + cbCardIndex[m_cbKingCardIndex[1]])==1)
		{
			//变量定义
			tagAnalyseItem AnalyseItem;
			ZeroMemory(&AnalyseItem,sizeof(AnalyseItem));
	...
	if (cbCardCount>=3)
	{
		for (BYTE i=0;i<MAX_INDEX;i++)
		{
			...
			if (cbCardIndex[i]>=3)
			{
				...
				KindItem[cbKindItemCount].cbWeaveKingReplace=0;
	...
	if (cbCardCount>=3)
	{
		...
		if(cbTotalKingCount>=3)
		{
			if(cbKingCount>0&&cbViceKingCount>0)
			{
				...
				KindItem[cbKindItemCount].cbWeaveKingReplace=1;
				KindItem[cbKindItemCount++].wWeaveKind=WIK_PENG;
			}
		}
		for (BYTE i=0;i<MAX_INDEX;i++)
...
WORD CGameLogic::AnalyseChiHuCard(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount, BYTE cbCurrentCard, WORD wChiHuRight, tagChiHuResult & ChiHuResult,bool bZimo)
{
	...
	//德国七星
	BYTE bGerman=0;
	if(IsQiXiaoDui(cbCardIndexTemp,WeaveItem,cbWeaveCount,cbCurrentCard,bGerman))
	{
		if(bGerman>0)
	...
	//7星
	if(ServenStar(cbCardIndexTemp)==true)
	{
		if(cbCardIndexTemp[m_cbKingCardIndex[0]]+cbCardIndexTemp[m_cbKingCardIndex[1]]==0)
	...
	//13离
	if(cbWeaveCount==0 &&IsNeatAlone(cbCardIndexTemp)==true)
	{
		if(cbCardIndexTemp[m_cbKingCardIndex[0]]+cbCardIndexTemp[m_cbKingCardIndex[1]]==0)
			wChiHuRight |=CHR_GERMAN;
	...
	if((bZimo||wChiHuRight&CHR_DI) &&(cbCardIndex[m_cbKingCardIndex[0]]>0))
	{
		...
		if(IsQiXiaoDui(cbCardIndexTemp2,WeaveItem,cbWeaveCount,cbCurrentCard,cbGerman)==true)
		{
			if(cbGerman==1&&(bCurrentKing==false || SwitchToCardIndex((cbCurrentCard))==m_cbKingCardIndex[0]))
				wChiHuRight |= CHR_GERMAN;
			ChiHuResult.wChiHuKind = CHK_QI_DUI;
			ChiHuResult.wChiHuRight =( CHR_GERMAN|CHR_KING_WAIT|wChiHuRight);
			return WIK_CHI_HU;
	...
	//精掉
	if(bZimo &&(cbCardIndex[m_cbKingCardIndex[1]]>0))
	{
		BYTE cbCardIndexTemp2[MAX_INDEX];
		CopyMemory(cbCardIndexTemp2,cbCardIndexTemp,sizeof(cbCardIndexTemp2));
		RemoveCard(cbCardIndexTemp2,cbCurrentCard);
		...
		if(IsQiXiaoDui(cbCardIndexTemp2,WeaveItem,cbWeaveCount,cbCurrentCard,cbGerman)==true)
		{
			if(cbGerman==1&&(bCurrentKing==false || SwitchToCardIndex((cbCurrentCard))==m_cbKingCardIndex[1]))
				wChiHuRight |= CHR_GERMAN;
			ChiHuResult.wChiHuKind = CHK_QI_DUI;
			ChiHuResult.wChiHuRight =( CHR_GERMAN|CHR_KING_WAIT|wChiHuRight);
			return WIK_CHI_HU;
...
bool CGameLogic::IsNeatAlone(BYTE cbCardIndex[MAX_INDEX])
{
	...
	for(i=0; i<3; i++)
	{
		...
		{
			//过滤精牌
			if(j==m_cbKingCardIndex[0]) continue;
			if(j==m_cbKingCardIndex[1]) continue;
...
//3顺 3刻
bool CGameLogic::IsNeat3(BYTE cbCardIndex[MAX_INDEX],BYTE &cbGerman)
{
	//德国
	cbGerman=1;
...
//七小对牌
bool CGameLogic::IsQiXiaoDui(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], const BYTE cbWeaveCount,const BYTE cbCurrentCard,BYTE cbGerman)
{
	//组合判断
	if (cbWeaveCount!=0) return false;
...
//7星
bool CGameLogic::ServenStar(BYTE cbCardIndex[MAX_INDEX])
{
	...
	//全不靠
	bool bRet = false;
	bool bAlone=IsNeatAlone(cbCardIndexTemp);
	if (bAlone==false)
	{
		return false;
	}
