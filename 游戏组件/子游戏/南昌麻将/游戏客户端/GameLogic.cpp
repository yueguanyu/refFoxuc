	...
	else
	{
		...
		if (cbCardCount>=3)
		{
			...
			for (BYTE i=0;i<MAX_INDEX;i++)
			{
				...
				//连牌判断
				if ((i<(MAX_INDEX-2-7))&&(cbTempCardIndex[i]>0)&&((i%9)<7))
				{
					...
					if (cbTempCardIndex[i]+cbTempCardIndex[i+1]+cbTempCardIndex[i+2]+cbTotalKingCount>=3)
					{
						...
						while (cbTempCardIndex[0]+cbTempCardIndex[1]+cbTempCardIndex[2]+cbTotalKingCount>=3)
						{
							//过滤3精
							if(cbIndex[0]+cbIndex[1]+cbIndex[2]==0) break;
		...
		if (cbKindItemCount>=cbLessKindItem)
		{
			...
			do
			{
				...
				if (bEnoughCard==true)
				{
					...
					if (cbKingCount==1&& cbViceKingCount==1) //正精+副精
					{
						//精牌替换
						cbEyeKingReplace=1;	
					...
					if(bContinue==true)
					{
						if ((cbViceKingCount  + cbKingCount)==1) //精牌+单张
						{
...
bool CGameLogic::IsQiXiaoDui(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], const BYTE cbWeaveCount,const BYTE cbCurrentCard,BYTE cbGerman,bool bZimo)
{
	...
	//放炮判断  财神
	if(bZimo==false &&(IsKingCardIndex(cbCurrentIndex)||IsKingCardIndex(cbCurrentIndex)))
	{
		//精牌不够
