...
LRESULT  CGameClientEngine::OnOpenSound(WPARAM wParam, LPARAM lParam)
{
	...
	if(IsEnableSound())
	{
		m_GameClientView.m_btSoundClose.ShowWindow(SW_HIDE);
		m_GameClientView.m_btSoundOpen.ShowWindow(SW_SHOW );

	}else
	{
		m_GameClientView.m_btSoundClose.ShowWindow( SW_SHOW);
		m_GameClientView.m_btSoundOpen.ShowWindow(SW_HIDE );
	}
	return 1;
...
bool CGameClientEngine::OnEventGameMessage(WORD wSubCmdID, VOID * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_GAME_END:		//游戏结束
		{
			while(m_PlaceJettonArray.GetCount()>0)
			{
				...
				OnSubPlaceJetton(&PlaceJetton,sizeof(PlaceJetton),false);
	...
	case SUB_S_TO_BANKER_MAX:
		{
			m_pIStringMessage->InsertNormalString(TEXT("本局押注达到庄家上限!"));
			return true;
		}
	...
	//MessageBox(TEXT("设置控制成功，仅当局有效！"));
		...
		{
			m_GameClientView.m_pAdminControl->ReqResult(pBuffer);
		}
	...
	case SUB_S_SEND_ACCOUNT:		//设置
		{
			...
			for (int i=0; i<pResult->m_UserCount; i++ )
			{
				...
				CString strName;
				strName.Format(TEXT("%s"),pUserData->GetNickName());
				m_GameClientView.m_pAdminControl->SetUserNickName(strName);
			}
	...
	case SUB_S_ADMIN_CHEAK:
		{
			...
			for (int nAreaIndex=0; nAreaIndex<AREA_COUNT; ++nAreaIndex)
				m_GameClientView.m_pAdminControl->SetUserGameScore(nAreaIndex,pResult->lUserJettonScore[nAreaIndex]);
			return true;
		}
	}
...
bool CGameClientEngine::OnSubPlaceJetton(const void * pBuffer, WORD wDataSize,bool bGameMes)
{
	...
	if (pPlaceJetton->cbAndroidUser==FALSE && (m_GameClientView.m_pAdminControl->GetSafeHwnd())&&bGameMes)
	{
		m_GameClientView.m_pAdminControl->SetUserBetScore(pPlaceJetton->cbJettonArea,pPlaceJetton->lJettonScore);
	}
...
void CGameClientEngine::UpdateButtonContron()
{
...
#ifdef _DEBUG
		TCHAR TszBuf[256];
		_snprintf(TszBuf,256,"lLeaveScore = %I64d lUserMaxJetton %I64d \n",lLeaveScore,lUserMaxJetton);
	...
	m_GameClientView.m_blCanStore = m_GameClientView.m_btBankerStorage.IsWindowEnabled();
...
LRESULT CGameClientEngine::OnAdminCommand(WPARAM wParam,LPARAM lParam)
{
	...
	SendSocketData(SUB_C_AMDIN_COMMAND,&Application,sizeof(CMD_C_AdminReq));
