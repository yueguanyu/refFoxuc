...
//配置文件
#define CONFIG_FILE_NAME            TEXT("NineXiaoBattleConfig.ini")
//////////////////////////////////////////////////////////////////////////
	...
	case GER_USER_LEAVE:		//用户离开
		{
			...
			if (m_pITableFrame->GetGameStatus()!=GS_GAME_END)
			{
				...
				//派发扑克
				if(m_AdminCommand.bCommandType != ADMIN_COMMAND_TYPE_NONECONTROL)
				{
					AdminControl();
					m_AdminCommand.bCommandType = ADMIN_COMMAND_TYPE_NONECONTROL;
				}
				else
				{
					bool bSuccess = false;
	...
	case IDI_GAME_END:			//结束游戏
		{
			...
			for ( WORD wUserChairID = 0; wUserChairID < GAME_PLAYER; ++wUserChairID )
			{
				...
				if (m_lUserWinScore[wUserChairID]!=0L) 
				{
					...
					m_lUserWinScore[wUserChairID] = 0L;
				}
			}
	...
	case SUB_C_AMDIN_COMMAND:
		{
			...
			//TCHAR str[222];
			//myprintf(str,_T("%s"), m_pServerContro == NULL ? _T("bu"):_T("zai"));
			//AfxMessageBox(str);
...
bool CTableFrameSink::ProbeJudge()
{
	...
	//区域倍率
	BYTE const cbMultiple[] = {0, 1, 1, 1, 1, 1, 1};
	...
	//角标识
	bool bWinBankerJiaoL,bWinBankerJiaoR,bWinBankerQiao;
	bWinBankerJiaoL=(false==bWinShunMen && false==bWinDuiMen) ? true : false;
	bWinBankerJiaoR=(false==bWinDaoMen && false==bWinDuiMen) ? true : false;
	bWinBankerQiao=(false==bWinShunMen && false==bWinDaoMen) ? true : false;
...
//管理控制
void CTableFrameSink::AdminControl()
{
	//派发扑克
	DispatchTableCard();

	//庄家控制
	if(m_AdminCommand.bCommandType == ADMIN_COMMAND_TYPE_BANKCONTROL)
	{
		//庄家赢
		if(m_AdminCommand.bBankWin)
		{
			//大小比较
			for(BYTE bIndex = SHUN_MEN_INDEX; bIndex <= DAO_MEN_INDEX; bIndex++)
			{
				if(m_GameLogic.CompareCard(m_cbTableCardArray[BANKER_INDEX],2,m_cbTableCardArray[bIndex],2) == 1)
				{
					BYTE bTempCard[2] = {0};
					CopyMemory(bTempCard, m_cbTableCardArray[bIndex], sizeof(BYTE)*2);
					CopyMemory(m_cbTableCardArray[bIndex], m_cbTableCardArray[BANKER_INDEX], sizeof(BYTE)*2);
					CopyMemory(m_cbTableCardArray[BANKER_INDEX], bTempCard, sizeof(BYTE)*2);
				}
			}
		}
		else
		{
			...
			for(BYTE bIndex = SHUN_MEN_INDEX; bIndex <= DAO_MEN_INDEX; bIndex++)
			{
				if(m_GameLogic.CompareCard(m_cbTableCardArray[bIndex],2,m_cbTableCardArray[BANKER_INDEX],2) == 1)
				{
					BYTE bTempCard[2] = {0};
					CopyMemory(bTempCard, m_cbTableCardArray[bIndex], sizeof(BYTE)*2);
					CopyMemory(m_cbTableCardArray[bIndex], m_cbTableCardArray[BANKER_INDEX], sizeof(BYTE)*2);
	...
	//区域控制
	else if(m_AdminCommand.bCommandType == ADMIN_COMMAND_TYPE_AREACONTROL)
	{
		//大小比较 赢区域要最大牌
		for(BYTE bIndex = BANKER_INDEX; bIndex <= DAO_MEN_INDEX; bIndex++)
		{
			if(bIndex != m_AdminCommand.bWinArea)
			{
				if(m_GameLogic.CompareCard(m_cbTableCardArray[m_AdminCommand.bWinArea],2,m_cbTableCardArray[bIndex],2) == 1)
				{
					BYTE bTempCard[2] = {0};
					CopyMemory(bTempCard, m_cbTableCardArray[bIndex], sizeof(BYTE)*2);
					CopyMemory(m_cbTableCardArray[bIndex], m_cbTableCardArray[m_AdminCommand.bWinArea], sizeof(BYTE)*2);
					CopyMemory(m_cbTableCardArray[m_AdminCommand.bWinArea], bTempCard, sizeof(BYTE)*2);
				}
			}
		}

		//因只要一个区域赢，所以
		if(m_AdminCommand.bWinArea != BANKER_INDEX)
		{
			//大小比较
			for(BYTE bIndex = SHUN_MEN_INDEX; bIndex <= DAO_MEN_INDEX; bIndex++)
			{
