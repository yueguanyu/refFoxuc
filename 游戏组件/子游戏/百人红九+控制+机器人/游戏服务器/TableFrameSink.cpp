...
bool CTableFrameSink::OnUserPlaceJetton(WORD wChairID, BYTE cbJettonArea, LONGLONG lJettonScore)
{
	...
	if (bPlaceJettonSuccess)
	{
		...
		IServerUserItem * pIServerUserItem=m_pITableFrame->GetTableUserItem(wChairID);
		if (pIServerUserItem != NULL) 
		PlaceJetton.bIsAndroid=pIServerUserItem->IsAndroidUser()?TRUE:FALSE;
...
bool CTableFrameSink::DispatchTableCard()	
{
	...
	if (pUserItem != NULL && (pUserItem->GetUserData()->dwUserRight&UR_GAME_CONTROL) != 0)
		...
		if(pIServerUserItem!= NULL)
		{
			...
			if (wBankerTime+m_wAddTime<=m_wBankerTime || lBankerScore<m_lApplyBankerCondition)
			{
				...
				if (lBankerScore<m_lApplyBankerCondition)
					...
					_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("[ %s ]做庄次数达到(%d)，强行换庄!"),pIServerUserItem->GetNickName(),m_nBankerTimeLimit);
