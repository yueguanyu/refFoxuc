...
{	
	...
	for( ; lFirstScore != 0 ; )
	{
		...
		//过滤分数与第一个相反的筹码
		if( (lFirstScore>0L&&jp.lScore<0L) || (lFirstScore<0L&&jp.lScore>0L) ) 
			break;
...
BOOL CJettonControl::DistillJetIndex( LONGLONG lScore,INT_PTR &nJetIndex )
{
	...
	do
	{
		...
		while( lScore < m_lJetonIndex[wJettonIndex] )
		{
			nLastIndex++;
			if( nLastIndex == m_arJetIndex.GetCount() ) break;
			wJettonIndex = m_arJetIndex[nLastIndex];
		}
...
VOID CJettonControl::DrawJettonControl(CDC * pDC)
{
	...
	{
		//绘画筹码,只画最后MAX_DRAW
		if( m_arIndexPending.GetCount() > MAX_DRAW*2 )
			i = m_arIndexPending.GetCount() - MAX_DRAW*2;
		else i = 0;
