...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if(m_blGameEnd)
		{		m_ImageCard.BitBlt(pDC->GetSafeHdc(),OriginPoint.x+nXDrawPos+15,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
		...
		}else
		{
			   if(i<3)
			   {
				   m_ImageCard.BitBlt(pDC->GetSafeHdc(),OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
				...
				if(4==i)
				{
					m_ImageCard.BitBlt(pDC->GetSafeHdc(),OriginPoint.x+nXDrawPos+m_MovePoint.x+25,OriginPoint.y+nYDrawPos+m_MovePoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
					...
					m_ImageOpenCardHandle.TransDrawImage(pDC,OriginPoint.x-7,OriginPoint.y+nYDrawPos-8,m_ImageOpenCardHandle.GetWidth(),m_ImageOpenCardHandle.GetHeight(),0,0,RGB(255,0,255));
				}
				else
				{
					m_ImageCard.BitBlt(pDC->GetSafeHdc(),OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
