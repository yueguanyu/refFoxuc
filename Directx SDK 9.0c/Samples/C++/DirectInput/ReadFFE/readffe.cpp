...
HRESULT OnReadFile( HWND hDlg )
{
    ...
    static TCHAR strSearch[MAX_PATH] = TEXT("");

    // Search around for directory of .ffe files
    ...
    if( strLastSlash )
        *strLastSlash = 0;
    wsprintf( strSearch, TEXT("%s\\bazooka.ffe"), strExePath );
    if( GetFileAttributes( strSearch ) == 0xFFFFFFFF )
    {
        wsprintf( strSearch, TEXT("%s\\..\\bazooka.ffe"), strExePath );
        if( GetFileAttributes( strSearch ) == 0xFFFFFFFF )
        {
            ...
            if( GetFileAttributes( strSearch ) == 0xFFFFFFFF )
            {
                wsprintf( strSearch, TEXT("%s\\..\\ReadFFE\\bazooka.ffe"), strExePath );
                if( GetFileAttributes( strSearch ) == 0xFFFFFFFF )
                {
                    wsprintf( strSearch, TEXT("%s\\bazooka.ffe"), strExePath ); 
                }
    ...
    strLastSlash = _tcsrchr( strSearch, TEXT('\\') );
    if( strLastSlash )
        *strLastSlash = 0;
    ...
    OPENFILENAME ofn = { sizeof(OPENFILENAME), hDlg, NULL,
                         ...
                         0, 1, strFileName, MAX_PATH, NULL, 0, strSearch,
    ...
    if( '\0' == strPath[0] )
    {
        ...
        strLastSlash = _tcsrchr( strPath, TEXT('\\') );
        if( strLastSlash )
            *strLastSlash = 0;
    }
