...
VOID UpdateBehaviorText( HWND hDlg )
{
    ...
    if( bAllocSW )
    {
        wcscpy( strExcepted, L"The new sound will be played in software" );
    }

    if( bAllocHW )
    {
        wcscpy( strExcepted, L"The new sound will be played in hardware" );
    }
    ...
    if( bByTime )
    {
        if( bByPriority )
        {
            ...
            {
                wcscat( strExcepted, L", and if the hardware has no available "
    ...
    else
    {
        ...
        {
            wcscat( strExcepted, L", and the buffer will not steal any "
