                          ...
                          DWORD dwPrecision = 1 )
{
    ...
    {
        case 0:
            swprintf( tszstr, L"%.0f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf( tszstr, L"%.0f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf( tszstr, L"%.0f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
        case 1:
            swprintf( tszstr, L"%.1f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf( tszstr, L"%.1f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf( tszstr, L"%.1f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
        default:
        case 2:
            swprintf( tszstr, L"%.2f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf( tszstr, L"%.2f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            swprintf( tszstr, L"%.2f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
    }
...
VOID LoadParameterUI( HWND hwndDlg, DWORD dwFXType )
{
    ...
    swprintf( tszstr, L"Parameters for [ %s ]", g_tszFXNames[dwFXType] );
...
HRESULT ValidateWaveFile( HWND hDlg, TCHAR* strFileName )
{
    ...
    else // The load call succeeded
    {
        ...
        wcscpy( g_tszFilename, strFileName );
        return S_OK;
    }
}
