...
BEGIN
	...
	SELECT @TempID = TempID FROM (
		...
		FROM News
		WHERE IsLock=1 AND IsDelete=0 and classid<3) AS TC
	...
	ELSE IF @dwMode=2
	BEGIN
		SELECT top 1 NewsID, PopID, Subject, Subject1, OnTop, OnTopAll, IsElite, IsHot, IsLock, IsDelete, IsLinks, LinkUrl, Body, FormattedBody, HighLight, ClassID, UserID, IssueIP, LastModifyIP, IssueDate, LastModifyDate FROM (
			SELECT ROW_NUMBER() OVER(ORDER By OnTopAll DESC,OnTop DESC,IssueDate DESC ,NewsID DESC) AS TempID, * 
			FROM News
			WHERE IsLock=1 AND IsDelete=0 and classid<3 ) AS TC
		WHERE TC.TempID = @TempID + 1		
